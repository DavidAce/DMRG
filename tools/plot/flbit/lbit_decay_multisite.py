...
from dmrg_plot.flbit.plotting.tools import *
...
def plot_lbits(meta, figs=None, color=None):
    ...
    for fidx, fval in enumerate(frange):
        for uidx, uval in enumerate(urange):
            ...
            # print(popt, pstd)
            #
            # ax.plot(x + site, fit_exp_stretched(x, popt[0], popt[1], popt[2]), label=None, color='green',
            #         path_effects=pe_expfit, zorder=60, )
            # ax.plot(x + site, exp_fit(x, popt[0], popt[1]), label=None, color=color, path_effects=pe_expfit)
                for rep in range(maxreps):
                    l = lbits[fidx, uidx, rep, [site], :].T
                    y = np.mean(l, axis=1)
                    x = range(len(y))
                    e = np.std(l, axis=1)
                    y[y < 1e-15] = np.nan
                    line, = ax.plot(x, y, alpha=0.30, color=color)
                    # line, = ax.plot(x, y, linewidth=1.0, color='red', alpha=1.0)
