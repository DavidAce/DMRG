...
for h5file, marker, in h5files_new:
    for g in gperts:
        ...
        with h5py.File(h5file, 'r') as f:
            for key, val in f.items():
                ...
                if 'energy_upper_bound' in val.dtype.names:
                    ene_max = val['energy_upper_bound'][()]
                ...
                var_abs_chn = np.median(np.abs(var_prt - var_old)/g_pert**2/(2*ene_max))
                var_rel_chn = np.median(np.abs(var_prt - var_old) / var_prt/(2*ene_max))
                ...
                yA1vals.append(var_abs_chn)
                eA1vals.append(var_abs_chn_ste)
                yA1titl = f'Absolute change in energy variance, normalized by energy bandwidth, $g = 0\\rightarrow 10^{{{int(np.log10(g_pert))}}}$'
                yA1labl = "${\\mathrm{median}(|\sigma_{H}^2 - \sigma_{H^\prime}^2|/(g^2* E_\mathrm{bandwidth})}$"

                yB1vals.append(var_rel_chn)
                eB1vals.append(var_rel_chn_ste)
                yB1titl = f'Relative change in energy variance $g = 0\\rightarrow 10^{{{int(np.log10(g_pert))}}}$'
                yB1labl = "${\\mathrm{median}(|\sigma_{H}^2 - \sigma_{H^\prime}^2|/{\sigma_{H}^2})}$"
                ...
                ene_abs_chn = np.median(np.abs(ene_prt - ene_old)/np.abs(2*ene_max))
                ene_rel_chn = np.median(np.abs(ene_prt - ene_old)**2 /g_pert/np.abs(2*ene_max)/ np.abs(ene_old))
                ene_rel_ste = np.std(np.abs(ene_old - ene_prt)) / np.sqrt(ene_num)
                ene_abs_ste = np.std(np.abs(ene_prt - ene_old)) / np.sqrt(ene_num)

                yA2vals.append(ene_abs_chn)
                eA2vals.append(ene_abs_ste)
                yA2titl = f'Absolute change in energy $g = 0\\rightarrow 10^{{{int(np.log10(g_pert))}}}$'
                yA2labl = "$\\mathrm{median}(|E - E^\prime|)$"


                yB2vals.append(ene_rel_chn)
                eB2vals.append(ene_rel_ste)
                yB2titl = f'Relative change in energy $g = 0\\rightarrow 10^{{{int(np.log10(g_pert))}}}$'
                yB2labl = "$\\mathrm{median}(|E - E^\prime|^2/(g \cdot E \cdot E_\mathrm{bandwidth})$"
        ...
        lineA1 = axes1['A'].errorbar(x=xvals, y=yA1vals, yerr=None, marker=marker)
        ...
        lineB1 = axes1['B'].errorbar(x=xvals, y=yB1vals, yerr=None, marker=marker)
        ...
        lineC1 = axes1['C'].errorbar(x=xvals, y=yC1vals, yerr=None, marker=marker)
    ...
    for d in reversed(deltas):
        ...
        with h5py.File(h5file, 'r') as f:
            for key, val in f.items():
                ...
                yA3vals.append(var_abs_chn/L/g_pert/g_pert)
                eA3vals.append(var_abs_chn_ste/L/g_pert/g_pert)
                yA3titl = f'Absolute change in energy variance'
                yA3labl = "${\\mathrm{median}(\sigma_{H^\prime}^2 - \sigma_{H}^2)}/{L}$"

                yB3vals.append(var_rel_chn*g_pert)
                eB3vals.append(var_rel_chn_ste*g_pert)
                yB3titl = f'Relative change in energy variance'
                yB3labl = "${\\mathrm{median}(\sigma_{H^\prime}^2 - \sigma_{H}^2)}/{\sigma_{H}^2 L}$"


                yC3vals.append(var_frc_chn)
                eC3vals.append(var_frc_chn_ste)
                yC3titl = f'Ratio change in energy variance '
                ...
                ene_prt = val['energy_pert'][()]
                enech_avg = np.median(np.abs(ene_old - ene_prt))
                ene_abs_chn = np.mean(np.abs(ene_prt - ene_old))
                ene_rel_chn = np.mean(np.abs(ene_prt - ene_old))
                ene_frc_chn = np.mean(np.abs(ene_prt/ene_old))
                ene_frc_ste = np.std(np.abs(ene_prt/ene_old)) / np.sqrt(ene_num)
                ene_rel_ste = np.std(np.abs(ene_old - ene_prt)) / np.sqrt(ene_num)
                ene_abs_ste = np.std(np.abs(ene_prt - ene_old)) / np.sqrt(ene_num)

                yD3vals.append(ene_rel_chn/g_pert**2)
                eD3vals.append(ene_rel_ste/g_pert**2)
                yD3titl = f'Relative change in energy$'
                yD3labl = "${\\mathrm{median}(|E' - E|/|E|)}$"
        ...
        eD3vals = np.asarray(eD3vals)[sort]
        ...
        lineA1 = axes3['A'].errorbar(x=xvals, y=yA3vals- yA3vals[-1], yerr=None,  marker=marker)
        lineA1.set_label(label)
        ...
        print(f"{yD3vals[-1]=:.16f}")
        lineD3 = axes3['D'].errorbar(x=xvals, y=yD3vals-yD3vals[-1], yerr=None, marker=marker)
