...
def plot_chain(h5, db, meta, sub1, l1):
    if len(sub1) != 1:
            ...
            for statekey in iterate(db['keys']['state'], stateinc):
                ...
                for pointkey in iterate(db['keys']['point'], pointinc):
                    ...
                    for key1 in get_keys(db, l1[0]):
                        ...
                        if not ygrps:
                            raise KeyError("Could not find ygrp {}. findlist: {}".format(meta['ygrp'], findlist))
    ...
    figname = "{}/{}_chain({})_sub({})_line({})".format(meta['plotdir'], meta['plotprefix'], meta['ycol'], sub1[0], l1[0])
