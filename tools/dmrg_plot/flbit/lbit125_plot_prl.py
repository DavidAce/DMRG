...
from plotting.meta125_prl import *
...
def lbit_plot(args):
    ...
    for idx, (db, meta, palette) in enumerate(zip(dbs, metas, palettes)):
        f = plot_tavg_fig_sub_line(db=db, meta=meta['tavg-num-minmax'], figspec=figspec_L, subspec=subspec_L,
                                   linspec=linspec_L, xaxspec=xaxspec_L, figs=f, palette_name=palette,dbidx=0,dbnum=1)
    save_figure(f)
    plt.show()
    exit(0)
...
if __name__ == '__main__':
    args = parse('fLBIT', ['lbit125'],)#" basedir='/mnt/wdpool/backup/lbit')
    #lbit_avg(args)
    lbit_plot(args)
