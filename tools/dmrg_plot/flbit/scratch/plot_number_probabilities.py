...
d = 0
fieldinset=True
cachedir = "/mnt/WDB-AN1500/mbl_transition/lbit93-precision/analysis/cache"
analysisfile = "/mnt/WDB-AN1500/mbl_transition/lbit93-precision/analysis/data/averaged.h5"
simpath = "L24/J[+0±1_+0±1_+0±1]/x1/rL/u[d16_f0.2_tw1.0ID_cw1.0EX_bond0]/fLBIT"

with h5py.File(analysisfile, 'r') as h5file:
    ...
    db = {'vals': {}}
    db['vals']['L'] = L
    db['vals']['r'] = 'L' #hamiltonian['J2_span'][()]
    db['vals']['x'] = 1.0 #hamiltonian['xi_Jcls'][()]
    db['vals']['f'] = 0.2 #hamiltonian['u_fmix'][()]
    db['vals']['l'] = 1.0
    db['vals']['w'] = (1.0,1.0,1.0)
    db['vals']['cachedir'] = cachedir
    db['vals']['filename'] = analysisfile
    ...
    for d,palette,label in zip([0, -1, 1, 2,-2], ['Blues', 'Oranges', 'Greens', 'Purples', 'Reds'],['$p(L/4)$','$p(L/4-1)$','$p(L/4+1)$','$p(L/4-2)$','$p(L/4+2)$']):
        ...
        for tidx in time_indices:
            print(f'{L//4=} | {d=} | {tidx=}')
            hist, edge = np.histogram(p[int(L//4) + d, int(L//2), tidx, :], bins=100, range=(0,1), density=True)
