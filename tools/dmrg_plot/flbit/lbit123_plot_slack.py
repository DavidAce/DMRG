...
from plotting.meta123_slack import *
            ...
            for avgfile in [
                ...
                if version3:
                    ...
                    dbs.append(load_time_database3(h5avgs[-1], metas[-1], algo_filter=algo_filter, model_filter=model_filter,
                                                   state_filter=[state_filter[0]], debug=True))
                    metas.append(get_meta(plotdir, cachedir))
                    dbs.append(load_time_database3(h5avgs[-1], metas[-1], algo_filter=algo_filter, model_filter=model_filter,
                                                   state_filter=[state_filter[1]], debug=True))
...
if __name__ == '__main__':
    args = parse('fLBIT', ['lbit123'],states=['state_real','state_eff'])# basedir='/mnt/wdpool/backup/lbit')
    #lbit_avg(args)
    lbit_plot(args)
