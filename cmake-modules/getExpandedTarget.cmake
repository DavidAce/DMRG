function(expandTargetLibs target_names expanded_list)
    set(scoped_list)
...
#            message("Iterating through: ${target_libs}")
            foreach(lib ${target_libs})
                if(TARGET ${lib})
                    expandTargetLibs(${lib} ${expanded_list})
                else()
...
function(expandTargetIncs target_names expanded_list)
    foreach(target_name ${target_names})
        if(TARGET ${target_name})
            get_target_property(target_libs  ${target_name} INTERFACE_LINK_LIBRARIES)
            ...
            foreach(lib ${target_libs})
                if(TARGET ${lib})
                    expandTargetIncs(${lib} ${expanded_list})
...
function(expandTargetOpts target_names expanded_list)
    foreach(target_name ${target_names})
        if(TARGET ${target_name})
            get_target_property(target_libs  ${target_name} INTERFACE_LINK_LIBRARIES)
            ...
            foreach(lib ${target_libs})
                if(TARGET ${lib})
                    expandTargetOpts(${lib} ${expanded_list})
