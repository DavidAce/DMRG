...
function(find_Arpackpp)
    if (NOT TARGET arpack::arpack)
        message(WARNING "Arpack++ needs to link to target [arpack::arpack]")
    endif()
    if (NOT TARGET lapacke::lapacke)
        message(WARNING "Arpack++ needs to link to target [lapacke::lapacke]")
    endif()
    ...
    if (NOT TARGET arpack::arpack++)
        ...
        if(ARPACKPP_INCLUDE_DIR)
            CheckArpackppCompiles("header_only" "-std=c++17 -g3"  ""  "" "${ARPACKPP_INCLUDE_DIR}" "arpack::arpack;lapacke::lapacke")
    ...
    if(NOT TARGET arpack::arpack++)
        ...
        if(ARPACKPP_LIBRARIES AND ARPACKPP_INCLUDE_DIR)
            CheckArpackppCompiles("lib" "-std=c++17 -g3"  ""  "${ARPACKPP_LIBRARIES}" "${ARPACKPP_INCLUDE_DIR}" "arpack::arpack;lapacke::lapacke")
            if(ARPACKPP_COMPILES_lib)
                ...
                add_library(arpack::arpack++ INTERFACE IMPORTED)
                target_link_libraries(arpack::arpack++ INTERFACE ${ARPACKPP_LIBRARIES} lapacke::lapacke arpack::arpack)
                target_include_directories(arpack::arpack++ SYSTEM INTERFACE ${ARPACKPP_INCLUDE_DIR})
            endif()
        else()
