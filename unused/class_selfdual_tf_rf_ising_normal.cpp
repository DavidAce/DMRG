...
class_selfdual_tf_rf_ising_normal::class_selfdual_tf_rf_ising_normal(size_t position_, std::string logName) : class_model_base(position_, logName) {
    spin_dim  = settings::model::selfdual_tf_rf_ising_normal::d; /*!< Spin dimension */
    J_mean    = settings::model::selfdual_tf_rf_ising_normal::J_mean;
    h_mean    = settings::model::selfdual_tf_rf_ising_normal::h_mean;
    J_sigma   = settings::model::selfdual_tf_rf_ising_normal::J_sigma;
    h_sigma   = settings::model::selfdual_tf_rf_ising_normal::h_sigma;
    lambda    = settings::model::selfdual_tf_rf_ising_normal::lambda;
    e_reduced = 0;
...
double class_selfdual_tf_rf_ising_normal::get_coupling() const { return std::pow(J_rnd + J_ptb, 1 - alpha); }
double class_selfdual_tf_rf_ising_normal::get_field() const { return std::pow(h_rnd + h_ptb, 1 - beta); }
double class_selfdual_tf_rf_ising_normal::get_coupling(double J_rnd_, double J_ptb_, double alpha_) const { return std::pow(J_rnd_ + J_ptb_, 1 - alpha_); }
double class_selfdual_tf_rf_ising_normal::get_field(double h_rnd_, double h_ptb_, double beta_) const { return std::pow(h_rnd_ + h_ptb_, 1 - beta_); }
...
void class_selfdual_tf_rf_ising_normal::set_hamiltonian(const Eigen::MatrixXd &all_parameters, int position) { set_hamiltonian(all_parameters.row(position)); }
...
void class_selfdual_tf_rf_ising_normal::set_hamiltonian(const Eigen::VectorXd &parameters) {
    ...
    delta                            = parameters(12);
    e_reduced                        = parameters(13);
    spin_dim                         = parameters(14);
...
void class_selfdual_tf_rf_ising_normal::set_perturbation(double coupling_ptb, double field_ptb, PerturbMode ptbMode) {
    switch(ptbMode) {
...
bool class_selfdual_tf_rf_ising_normal::is_perturbed() const { return J_ptb != 1.0 or h_ptb != 1.0; }
...
Eigen::Tensor<Scalar, 4> class_selfdual_tf_rf_ising_normal::MPO_reduced_view(double site_energy) const {
    ...
    temp.slice(Eigen::array<long, 4>{4, 0, 0, 0}, extent4).reshape(extent2) = Textra::MatrixTensorMap(-get_field() * sx - e_reduced * Id);
    temp.slice(Eigen::array<long, 4>{4, 1, 0, 0}, extent4).reshape(extent2) = Textra::MatrixTensorMap(-get_coupling() * sz);
    return temp;
}

Eigen::MatrixXcd class_selfdual_tf_rf_ising_normal::single_site_hamiltonian(int                            position,
                                                                            int                            sites,
...
Eigen::Tensor<Scalar, 1> class_selfdual_tf_rf_ising_normal::get_MPO_edge_left() const {
    Eigen::Tensor<Scalar, 1> ledge(5);
    ledge.setZero();
    ledge(4) = 1;
    return ledge;
}
Eigen::Tensor<Scalar, 1> class_selfdual_tf_rf_ising_normal::get_MPO_edge_right() const {
...
std::map<std::string, double> class_selfdual_tf_rf_ising_normal::get_parameters() const {
    /* clang-format off */
    ...
    parameters.insert({"J_mean", J_mean});
    parameters.insert({"h_mean", h_mean});
...
void class_selfdual_tf_rf_ising::set_full_lattice_parameters(std::vector<std::map<std::string, double>> chain_parameters, bool reverse) {
    if(reverse) {
