    ...
    foreach (fort ${MKL_FIND_FORTRAN_COMPONENT})
        foreach (thread ${MKL_FIND_THREAD_COMPONENT})
            foreach (arch ${MKL_FIND_ARCH_COMPONENT})
                ...
                if (blas IN_LIST MKL_FIND_MATH_COMPONENTS AND TARGET mkl::mkl_blas${MKL_95_SUFFIX}${arch})
                    ...
                    message(FATAL_ERROR "No target [mkl::mkl_blas${MKL_95_SUFFIX}${arch}]: library not found")
                endif ()
...
if(MKL_TARGETS_NUM EQUAL 1)
    ...
    target_compile_definitions(mkl::mkl INTERFACE MKL_Complex8=std::complex<float>)
    target_compile_definitions(mkl::mkl INTERFACE MKL_Complex16=std::complex<double>)
    if (blas IN_LIST MKL_FIND_COMPONENTS)
        add_library(BLAS::BLAS ALIAS mkl::mkl)
        message(STATUS "Defined target alias BLAS::BLAS -> mkl::mkl")
    endif ()
    if (lapack IN_LIST MKL_FIND_COMPONENTS)
        add_library(LAPACK::LAPACK ALIAS mkl::mkl)
        message(STATUS "Defined target alias LAPACK::LAPACK -> mkl::mkl")
    endif ()
else()
