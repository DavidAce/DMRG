...
if(tblis_FOUND AND NOT TARGET tblis::tblis)
    ...
    if(NOT BUILD_SHARED_LIBS)
        ...
        if(HWLOC_LIBRARY)
            ...
            set_property(TARGET ${PROJECT_NAME} APPEND PROPERTY BUILD_RPATH "$<TARGET_LINKER_FILE_DIR:hwloc::hwloc>")
            target_link_libraries(tblis::tblis INTERFACE hwloc::hwloc)
        endif()
        if(ATOMIC_LIBRARY)
            target_link_libraries(tblis::tblis INTERFACE ${ATOMIC_LIBRARY})
        else()
            message(WARNING "atomic library not found (tblis may depend on it). Linking with -latomic")
            target_link_libraries(tblis::tblis INTERFACE atomic)
        endif()
        if(UDEV_LIBRARY)
            target_link_libraries(tblis::tblis INTERFACE ${UDEV_LIBRARY})
        else()
            message(WARNING "udev library not found (tblis may depend on it)")
        endif()
    endif()
