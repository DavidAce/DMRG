...
if iDMRG_exists:
    ...
    plt.suptitle(group)
    ax[0].axhline(y=-1.2732395447351625, xmin=0, xmax=1, c="blue", linewidth=0.5, zorder=0,label='$\langle e\\rangle_{exact}, \chi = \infty$')
    for key in keys:
        chi_label =  '$\chi=$' + str( store[key].chi_max[0])
        plt_graph(key,"iteration", "energy1", "L", "E/L", ax[0], label="$\langle e \\rangle_{MPO}$")
        plt_graph(key,"iteration", "energy2", "L", "E/L", ax[0], label="$\langle e \\rangle = \langle h_{even} + h_{odd}  \\rangle$")
        # plt_graph(key,"iteration", "energy3", "L", "E/L", ax[0], label="$d\log{G1(a)} / d a$")
        plt_graph(key,"iteration", "energy4", "L", "E/L", ax[0], label="$\langle e \\rangle = d\log{G_\infty(a)} / d a$")
        ...
        plt_graph(key,"iteration", "variance1", "L", "$\sigma^2(E)/L$", ax[1], label="${Var}(H)_{MPO}/L$", scale = 'log')
        plt_graph(key,"iteration", "variance2", "L", "$\sigma^2(E)/L$", ax[1], label="${Var}(H)/L = \langle (\\tilde h_{even} + \\tilde h_{odd} )^2 \\rangle$"  , scale = 'log')
        # plt_graph(key,"iteration", "variance3", "L", "$\sigma^2(E)/L$", ax[1], label="$log(|G1(a)|^2) / a^2$"      , scale = 'log')
        plt_graph(key,"iteration", "variance4", "L", "$\sigma^2(E)/L$", ax[1], label="${Var}(H)/L =  log(|G_\infty(a)|^2) / a^2$"      , scale = 'log')
        ...
        # plt_graph(key,"iteration", "variance1", "L", "$\sigma^2(E)/L$", ax[0,1], label="MPO"                         , scale = 'linear')
        # plt_graph(key,"iteration", "variance2", "L", "$\sigma^2(E)/L$", ax[0,1], label="$\langle (H-E)^2 \\rangle_1$", scale = 'linear')
        # plt_graph(key,"iteration", "variance3", "L", "$\sigma^2(E)/L$", ax[0,1], label="$log(|G1(a)|^2) / a^2$"      , scale = 'linear')
