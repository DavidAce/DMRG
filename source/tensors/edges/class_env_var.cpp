...
void class_env_var::clear() {
    if(sites > 0) block.resize(0, 0, 0, 0); // = Eigen::Tensor<Scalar, 4>();
}
...
bool class_env_var::is_real() const { return Textra::isReal(block, "env2" + side); }

bool class_env_var::has_nan() const { return Textra::hasNaN(block, "env2" + side); }
...
void class_env_var::enlarge(const Eigen::Tensor<Scalar, 3> &MPS, const Eigen::Tensor<Scalar, 4> &MPO) {
    ...
    if(not has_block()) throw std::runtime_error(fmt::format("env_var pos {} has no block to enlarge", get_position()));
    ...
    if(side == "L") {
        ...
        if(MPS.dimension(0) != MPO.dimension(2))
            throw std::runtime_error(fmt::format("ENV2 L pos {} dimension mismatch: MPS dim[{}]:{} != MPO   dim[{})]:{}", position.value(), 0, MPS.dimension(0),
                                                 2, MPO.dimension(2)));
        if(MPS.dimension(1) != block.dimension(0))
            throw std::runtime_error(fmt::format("ENV2 L pos {} dimension mismatch: MPS dim[{}]:{} != block dim[{})]:{}", position.value(), 1, MPS.dimension(1),
                                                 0, block.dimension(0)));
        if(MPO.dimension(0) != block.dimension(2))
            throw std::runtime_error(fmt::format("ENV2 L pos {} dimension mismatch: MPO dim[{}]:{} != block dim[{})]:{}", position.value(), 0, MPO.dimension(0),
                                                 2, block.dimension(2)));

        sites++;
        block_enlarged = block.contract(MPS, Textra::idx({0}, {1}))
    ...
    if(side == "R") {
         ...
         */
        if(MPS.dimension(0) != MPO.dimension(2))
            throw std::runtime_error(fmt::format("ENV R pos {} dimension mismatch: MPS dim[{}]:{} != MPO   dim[{})]:{}", position.value(), 0, MPS.dimension(0),
                                                 2, MPO.dimension(2)));
        ...
        sites++;
        block_enlarged = block.contract(MPS, Textra::idx({0}, {2}))
