...
void TensorsInfinite::reset_to_random_product_state(std::string_view sector, bool use_eigenspinors, size_t bitfield) {
    eject_edges();
    state->clear_cache(); // Other caches can remain intact
    tools::infinite::mps::random_product_state(*state, sector, use_eigenspinors, bitfield);
    reset_edges();
}
...
void TensorsInfinite::merge_twosite_tensor(const Eigen::Tensor<Scalar, 3> &twosite_tensor, std::optional<svd::config> svd_cfg) {
    state->clear_cache();
    clear_measurements();
    tools::infinite::mps::merge_twosite_tensor(*state, twosite_tensor, svd_cfg);
