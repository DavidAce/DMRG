...
namespace settings {
    ...
    extern double   get_bond_grow_factor(AlgorithmType algo_type);
...
//        inline size_t verbosity     = 2;                            /*!< Level of verbosity desired [0-6]. Level 0 prints everything, 6 nothing. Level 2 or 3 is recommended for normal use */
    ...
    namespace strategy {
        ...
        inline bool          bond_quench_when_stuck      = false;                                  /*!< Reduce bond for a few iterations when stuck and increasing bond dimension would not help */
