...
template<typename Scalar>
Textra::Tensor<Scalar, 2>
class_SVD<Scalar>::pseudo_inverse(const Textra::Tensor<Scalar, 2> &tensor){
...
class_SVD<Scalar>::decompose(const Textra::Tensor<Scalar,2> &tensor) {
    Eigen::Map<const MatrixType> mat (tensor.data(), tensor.dimension(0), tensor.dimension(1));
    SVD.compute(mat, Eigen::ComputeThinU | Eigen::ComputeThinV);
    truncation_error = SVD.singularValues().tail(SVD.nonzeroSingularValues()-chi).squaredNorm();
    long chi = SVD.rank();
...
template<typename Scalar>
std::tuple<Textra::Tensor<Scalar, 2> ,Textra::Tensor<Scalar, 1>, Textra::Tensor<Scalar, 2> >
class_SVD<Scalar>::decompose(const Textra::Tensor<Scalar,2> &tensor, const long chi_max) {
    Eigen::Map<const MatrixType> mat (tensor.data(), tensor.dimension(0), tensor.dimension(1));
    SVD.compute(mat, Eigen::ComputeThinU | Eigen::ComputeThinV);
    truncation_error = SVD.singularValues().tail(SVD.nonzeroSingularValues()-chi).squaredNorm();
...
class_SVD<Scalar>::schmidt(const Textra::Tensor<Scalar,2> &tensor, long d, long chiL, long chi_max, long chiR) {
    Eigen::Map<const MatrixType> mat (tensor.data(), tensor.dimension(0), tensor.dimension(1));
    SVD.compute(mat, Eigen::ComputeThinU | Eigen::ComputeThinV);
    long chi            = std::min(SVD.rank(),chi_max);
    truncation_error = SVD.singularValues().tail(SVD.nonzeroSingularValues()-chi).squaredNorm();
...
template<typename Scalar>
std::tuple<Textra::Tensor<Scalar, 3> ,Textra::Tensor<Scalar, 1>, Textra::Tensor<Scalar, 3> >
class_SVD<Scalar>::schmidt(const Textra::Tensor<Scalar,2> &tensor, long d, long chiL, long chiR) {
    Eigen::Map<const MatrixType> mat (tensor.data(), tensor.dimension(0), tensor.dimension(1));
    SVD.compute(mat, Eigen::ComputeThinU | Eigen::ComputeThinV);
    long chi            = SVD.rank();
    truncation_error = SVD.singularValues().tail(SVD.nonzeroSingularValues()-chi).squaredNorm();
...
template<typename Scalar>
std::tuple <Textra::Tensor<Scalar, 3> ,Textra::Tensor<Scalar, 1>, Textra::Tensor<Scalar, 3> >
class_SVD<Scalar>::schmidt(const Textra::Tensor<Scalar,4> &tensor, long d, long chiL, long chiR) {
...
template<typename Scalar>
std::tuple<Textra::Tensor<Scalar, 3> ,Textra::Tensor<Scalar, 1>, Textra::Tensor<Scalar, 3> >
class_SVD<Scalar>::schmidt(const Textra::Tensor<Scalar,4> &tensor, long d, long chiL, long chi_max, long chiR) {
    Eigen::Map<const MatrixType> mat (tensor.data(), tensor.dimension(0)*tensor.dimension(1), tensor.dimension(2)*tensor.dimension(3));
    SVD.compute(mat, Eigen::ComputeThinU | Eigen::ComputeThinV);
    long chi            = std::min(SVD.rank(),chi_max);
    truncation_error = SVD.singularValues().tail(SVD.nonzeroSingularValues()-chi).squaredNorm();
