...
void flbit::resume() {
    ...
    for (const auto &state_prefix: state_prefixes) {
        ...
        switch (settings::strategy::initial_state) {
            ...
            case StateInit::PRODUCT_STATE_NEEL_DISLOCATED:
                break;
            default:
        ...
        tensors.initialize_state(ResetReason::INIT, settings::strategy::initial_state, StateInitType::REAL,
                                 settings::strategy::initial_axis,
...
void flbit::run_task_list(std::deque<flbit_task> &task_list) {
    ...
    while (not task_list.empty()) {
        ...
        switch (task) {
            case flbit_task::INIT_RANDOMIZE_MODEL:
                initialize_model();
                break;
            case flbit_task::INIT_RANDOMIZE_INTO_PRODUCT_STATE:
                initialize_state(ResetReason::INIT, StateInit::RANDOM_PRODUCT_STATE);
                break;
            ...
            case flbit_task::INIT_RANDOMIZE_INTO_PRODUCT_STATE_NEEL_DISLOCATED:
                ...
                break;
            case flbit_task::INIT_RANDOMIZE_INTO_ENTANGLED_STATE:
                initialize_state(ResetReason::INIT, StateInit::RANDOM_ENTANGLED_STATE);
                break;
            case flbit_task::INIT_RANDOMIZE_INTO_PRODUCT_STATE_PATTERN:
                initialize_state(ResetReason::INIT, StateInit::PRODUCT_STATE_PATTERN);
                break;
            case flbit_task::INIT_RANDOMIZE_INTO_MIDCHAIN_SINGLET_NEEL_STATE:
                ...
                break;
            case flbit_task::INIT_BOND_LIMITS:
                init_bond_dimension_limits();
                break;
            case flbit_task::INIT_TRNC_LIMITS:
                init_truncation_error_limits();
                break;
            case flbit_task::INIT_WRITE_MODEL:
                write_to_file(StorageEvent::MODEL);
                break;
            case flbit_task::INIT_CLEAR_STATUS:
                status.clear();
                break;
            case flbit_task::INIT_CLEAR_CONVERGENCE:
                clear_convergence_status();
                break;
            case flbit_task::INIT_DEFAULT:
                run_preprocessing();
                break;
            ...
            case flbit_task::TIME_EVOLVE:
                ...
                if (settings::flbit::run_iter_in_parallel)
                    ...
                    run_algorithm();
                break;
            case flbit_task::TRANSFORM_TO_LBIT:
                transform_to_lbit_basis();
                break;
            case flbit_task::TRANSFORM_TO_REAL:
                transform_to_real_basis();
                break;
            case flbit_task::POST_WRITE_RESULT:
                write_to_file(StorageEvent::FINISHED);
                break;
            case flbit_task::POST_PRINT_RESULT:
                print_status_full();
                break;
            case flbit_task::POST_PRINT_TIMERS:
                tools::common::timer::print_timers();
                break;
            case flbit_task::POST_FES_ANALYSIS:
                run_fes_analysis();
                break;
            case flbit_task::POST_DEFAULT:
                run_postprocessing();
                break;
            case flbit_task::TIMER_RESET:
                tid::reset("fLBIT");
                break;
        }
        task_list.pop_front();
    ...
    if constexpr (settings::debug) {
        if (tensors.get_length() <= 8) {
            ...
            if (settings::flbit::use_mpo_circuit) {
                state_real = qm::lbit::transform_to_real_basis(*state_lbit, unitary_gates_mpo_layers, ledge, redge,
                                                               svd_cfg);
            ...
            auto u_circuit_t =
                    qm::lbit::get_unitary_circuit_as_tensor(
                            unitary_gates_2site_layers); // TODO: For some reason this is a transpose away from our typical circuit
            ...
            if (not uadjoint_u.isIdentity(status.trnc_lim * 10))
                err += fmt::format("uadjoint_u is not an identity matrix");
...
void flbit::run_algorithm() {
    ...
    tools::log->info("Starting {} algorithm with model [{}] for state [{}]", status.algo_type_sv(),
                     enum2sv(settings::model::model_type),
    ...
    while (true) {
        ...
        print_status();
        tools::log->trace("Finished step {}, iter {}, pos {}, dir {}", status.step, status.iter, status.position,
                          status.direction);
    ...
    tools::log->info("Finished {} simulation of state [{}] -- stop reason: {}", status.algo_type_sv(),
                     tensors.state->get_name(), status.algo_stop_sv());
...
void flbit::run_algorithm_parallel() {
    ...
    auto ham_swap_gates = std::vector<std::vector<qm::SwapGate>>{ham_swap_gates_1body, ham_swap_gates_2body,
...
#pragma omp parallel for ordered schedule(dynamic, 1)
    for (size_t tidx = 0; tidx < time_points.size(); ++tidx) {
        ...
        auto [state_tevo, status_tevo] =
                flbit_impl::update_state(tidx, time_points[tidx], *state_lbit_init, gates_tevo,
                                         unitary_gates_2site_layers, status_init);
...
#pragma omp ordered
        {
            ...
            tools::finite::h5::save::simulation(*h5file, state_tevo, *tensors.model, *tensors.edges, status_tevo,
                                                CopyPolicy::OFF);
            status_tevo.event = StorageEvent::NONE;
        };
        tools::log->trace("Finished step {}, iter {}, pos {}, dir {}", status_tevo.step, status_tevo.iter,
                          status_tevo.position, status_tevo.direction);
...
#pragma omp parallel for schedule(dynamic, 16)
        for (size_t idx = 0; idx < bitseqs.size(); ++idx) {
            if (u_mbl.size() != settings::model::lbit::u_depth)
                throw except::logic_error("u_mbl.size() != u_depth. Has u_mbl been initialized?");
            if (u_and.size() != settings::model::lbit::u_depth)
                throw except::logic_error("u_and.size() != u_depth. Has u_and been initialized?");

            auto pattern =
                    fmt::format("b{}", tools::get_bitfield(sites.size(), bitseqs[idx],
                                                           BitOrder::Reverse)); // Sort states with "1" appearing left to right
            auto state_i = StateFinite(AlgorithmType::fLBIT, sites.size(), 0, 2);
            tools::finite::mps::init::set_product_state_on_axis_using_pattern(state_i, StateInitType::REAL, "z",
                                                                              pattern);
...
void flbit::update_state() {
    ...
    tools::log->debug("Starting fLBIT: iter {} | Δt = ({:.2e}, {:.2e})", status.iter, f128_t(std::real(delta_t)),
                      f128_t(std::imag(delta_t)));
    if (not state_lbit)
        throw except::logic_error("state_lbit == nullptr: Set the state in lbit basis before running an flbit step");
    if (not state_lbit_init) {
        state_lbit_init = std::make_unique<StateFinite>(*state_lbit);
        tools::finite::mps::normalize_state(*state_lbit_init, std::nullopt, NormPolicy::ALWAYS);
    }
...
void flbit::update_time_step() {
    ...
    if (settings::flbit::time_scale == TimeScale::LOGSPACED)
        if (cmp_t(status.delta_t.to_floating_point<cplx_t>(), 0.0))
            throw except::logic_error("Expected nonzero delta_t after time step update");
    tools::log->debug("Time step iter {} | Δt = {} | t = {:8.2e}", status.iter,
                      status.delta_t.to_floating_point<cplx>(),
...
void flbit::check_convergence() {
    ...
    tools::log->debug("Simulation report: converged {} | saturated {} | stuck {} | succeeded {} | has to stop {}",
                      ...
                      status.algorithm_saturated_for, status.algorithm_has_stuck_for, status.algorithm_has_succeeded,
...
void flbit::create_time_points() {
    ...
    tools::log->info("Creating time points ({},{}) -> ({},{})", settings::flbit::time_start_real,
                     settings::flbit::time_start_imag,
    ...
    if (time_is_real and time_is_imag)
        throw except::logic_error("time_start and time_final must both be either purely real or imaginary. Got:\n"
                                  "time_start = {:.8f}{:+.8f}\n"
                                  "time_final = {:.8f}{:+.8f}",
                                  f128_t(time_start.real()), f128_t(time_start.imag()), f128_t(time_final.real()),
                                  f128_t(time_final.imag()));
    time_points.reserve(settings::flbit::time_num_steps);
    if (settings::flbit::time_scale == TimeScale::LOGSPACED) {
        if (time_is_real) {
            for (const auto &t: num::LogSpaced(settings::flbit::time_num_steps, time_start.real(),
                                               time_final.real())) { time_points.emplace_back(t); }
        } else {
        ...
        if (time_is_real) {
            for (const auto &t: num::LinSpaced(settings::flbit::time_num_steps, time_start.real(),
                                               time_final.real())) { time_points.emplace_back(t); }
        } else {
    ...
    // Sanity check
    if (time_points.front().real() != settings::flbit::time_start_real)
        throw except::logic_error("Time start real mismatch");
    if (time_points.front().imag() != settings::flbit::time_start_imag)
        throw except::logic_error("Time start imag mismatch");
    if (time_points.back().real() != settings::flbit::time_final_real)
        throw except::logic_error("Time final real mismatch");
    if (time_points.back().imag() != settings::flbit::time_final_imag)
        throw except::logic_error("Time final imag mismatch");
    if (time_points.size() != settings::flbit::time_num_steps)
        throw except::logic_error("Got time_points.size():[{}] != settings::flbit::time_num_steps:[{}]",
                                  time_points.size(), settings::flbit::time_num_steps);
}
...
void flbit::create_hamiltonian_gates() {
    ...
    bool has_swap_gates =
            not ham_swap_gates_1body.empty() or not ham_swap_gates_2body.empty() or not ham_swap_gates_3body.empty();
    bool has_slow_gates = not ham_gates_1body.empty() or not ham_gates_2body.empty() or not ham_gates_3body.empty();

    if (has_swap_gates)
        tools::log->warn("Hamiltonian swap gates have already been constructed: Normally this is done just once.");
    if (has_slow_gates)
        tools::log->warn("Hamiltonian gates have already been constructed: Normally this is done just once.");
    ...
    if (settings::flbit::use_swap_gates) {
        ...
        for (auto pos: list_1body) {
            ...
            auto sites = num::range<size_t>(pos, pos + range +
                                                 1); // A list of site indices. +1 to include last site (in this case the only site)
            auto nbody = std::vector<size_t>{
                    1};                   // A list of included nbody interaction terms (1: on-site terms, 2: pairwise, and so on)
            auto spins = tensors.state->get_spin_dims(
                    sites);      // A list of spin dimensions for each site (should all be 2 for two-level systems)
        ...
        for (auto pos: list_3body) {
            ...
            auto sites = num::range<size_t>(pos,
                                            std::clamp<size_t>(pos + range + 1, 0ul, L)); // +1 to include last site
        ...
        for (auto posL: list_2body) {
            ...
            if (J2_ctof == 0) break;
            if (posR >= L) break;
            auto sites = num::range<size_t>(posL, posR +
                                                  1); // +1 to include last site, which is otherwise not included in range
            auto nbody = std::vector<size_t>{0,
                                             2};          // zero is a flag to enable compensation for double-counting
            auto spins = tensors.state->get_spin_dims(sites);
            tools::log->debug("Generating {}-body hamiltonian on sites {}", nbody, sites);
        ...
        for (auto posL: list_3body) {
            auto range = 2ul;                                                                    // Distance to next-nearest neighbor when 3 sites interact
            auto sites = num::range<size_t>(posL,
                                            std::clamp<size_t>(posL + range + 1, 0ul, L)); // +1 to include last site
            auto nbody = std::vector<size_t>{3};
            auto spins = tensors.state->get_spin_dims(sites);
            tools::log->debug("Generating {}-body hamiltonian on sites {}", nbody, sites);
...
void flbit::update_time_evolution_gates() {
    ...
    if (not has_swap_gates and not has_slow_gates)
        throw except::logic_error("Hamiltonian gates have not been constructed");
    if (has_swap_gates and has_slow_gates)
        tools::log->warn("Both swap/non-swap gates have been constructed: Normally only one type should be used");
    auto delta_t = status.delta_t.to_floating_point<cplx_t>();
    if (has_swap_gates) {
        auto t_upd = tid::tic_scope("upd_time_evo_swap_gates");
        tools::log->debug("Updating time evolution swap gates to iter {} | Δt = ({:.2e}, {:.2e})", status.iter,
                          f128_t(std::real(delta_t)),
                          f128_t(std::imag(delta_t)));
    ...
    if (has_slow_gates) {
        ...
        tools::log->debug("Updating time evolution gates to iter {} | Δt = ({:.2e}, {:.2e})", status.iter,
                          f128_t(std::real(delta_t)),
                          f128_t(std::imag(delta_t)));
...
void flbit::create_unitary_circuit_gates() {
    ...
    for (const auto &field: tensors.model->get_parameter("J1_rand"))
        fields.emplace_back(static_cast<double>(std::any_cast<real_t>(field)));
    ...
    if (settings::flbit::use_mpo_circuit) {
        ...
        for (const auto &ulayer: unitary_gates_2site_layers)
            unitary_gates_mpo_layers.emplace_back(qm::lbit::get_unitary_mpo_layer(ulayer));
...
void flbit::time_evolve_lbit_state() {
    ...
    svd_cfg.svd_rtn = svd::rtn::geauto;
    bool has_swap_gates =
            not time_swap_gates_1body.empty() or not time_swap_gates_2body.empty() or not time_swap_gates_3body.empty();
    bool has_slow_gates = not time_gates_1body.empty() or not time_gates_2body.empty() or not time_gates_3body.empty();
    if (has_swap_gates and has_slow_gates)
        throw except::logic_error("Both swap and non-swap time evolution gates found");
    if (not has_swap_gates and not has_slow_gates)
        throw except::logic_error("None of swap or non-swap time evolution gates found");
    auto delta_t = status.delta_t.to_floating_point<cplx_t>();
    if (has_swap_gates) {
        tools::log->debug("Applying time evolution swap gates Δt = ({:.2e}, {:.2e})", f128_t(std::real(delta_t)),
                          f128_t(std::imag(delta_t)));
        tools::finite::mps::apply_swap_gates(*state_lbit, time_swap_gates_1body, CircuitOp::NONE, GateMove::AUTO,
                                             svd_cfg);
        tools::finite::mps::apply_swap_gates(*state_lbit, time_swap_gates_2body, CircuitOp::NONE, GateMove::AUTO,
                                             svd_cfg);
        tools::finite::mps::apply_swap_gates(*state_lbit, time_swap_gates_3body, CircuitOp::NONE, GateMove::AUTO,
                                             svd_cfg);
    }
    if (has_slow_gates) {
        tools::log->debug("Applying time evolution gates Δt = ({:.2e}, {:.2e})", f128_t(std::real(delta_t)),
                          f128_t(std::imag(delta_t)));
    ...
    if constexpr (settings::debug) {
        ...
        if (has_swap_gates) {
            tools::log->debug("Applying time evolution swap gates backward Δt = ({:.2e}, {:.2e})",
                              f128_t(std::real(delta_t)), f128_t(std::imag(delta_t)));
            tools::finite::mps::apply_swap_gates(state_lbit_debug, time_swap_gates_3body, CircuitOp::ADJ,
                                                 GateMove::AUTO, svd_cfg);
            tools::finite::mps::apply_swap_gates(state_lbit_debug, time_swap_gates_2body, CircuitOp::ADJ,
                                                 GateMove::AUTO, svd_cfg);
            tools::finite::mps::apply_swap_gates(state_lbit_debug, time_swap_gates_1body, CircuitOp::ADJ,
                                                 GateMove::AUTO, svd_cfg);
        }
        if (has_slow_gates) {
            tools::log->debug("Applying time evolution gates backward Δt = ({:.2e}, {:.2e})",
                              f128_t(std::real(delta_t)), f128_t(std::imag(delta_t)));
            tools::finite::mps::apply_gates(state_lbit_debug, time_gates_3body, CircuitOp::ADJ, true, GateMove::AUTO,
                                            svd_cfg);
            tools::finite::mps::apply_gates(state_lbit_debug, time_gates_2body, CircuitOp::ADJ, true, GateMove::AUTO,
                                            svd_cfg);
            tools::finite::mps::apply_gates(state_lbit_debug, time_gates_1body, CircuitOp::ADJ, true, GateMove::AUTO,
                                            svd_cfg);
        }
...
void flbit::transform_to_real_basis() {
    ...
    if (settings::flbit::use_mpo_circuit) {
        *tensors.state = qm::lbit::transform_to_real_basis(*state_lbit, unitary_gates_mpo_layers, ledge, redge,
                                                           svd_cfg);
...
void flbit::transform_to_lbit_basis() {
    ...
    if (settings::flbit::use_mpo_circuit) {
        *state_lbit = qm::lbit::transform_to_lbit_basis(*tensors.state, unitary_gates_mpo_layers, ledge, redge,
                                                        svd_cfg);
    } else {
...
//    AlgorithmFinite::write_to_file(*state_lbit, *tensors.model, *tensors.edges, storage_event, copy_policy);
    ...
    if (h5file and storage_event == StorageEvent::MODEL) {
        ...
        if (nsamps > 0) {
            ...
            auto lbitSA = qm::lbit::get_lbit_support_analysis(uprop_default, udpths, ufmixs, ulambdas, uwkinds,
                                                              umkinds);
            if (h5file and settings::storage::dataset::lbit_analysis::policy == StoragePolicy::INIT) {
                ...
                auto shape_data =
                        std::vector<long>{safe_cast<long>(nsamps), safe_cast<long>(settings::model::model_size),
                ...
                h5file->writeAttribute("The operator support matrix O(i,j) = (1/2^L) Tr(tau_i^z sigma_j^z)",
                                       "/fLBIT/model/lbits/corrmat", "description");
                //                }
                ...
                h5file->writeAttribute("Site arithmetic average <<O(|i-j|)>>", "/fLBIT/model/lbits/corravg",
                                       "description");
                ...
                h5file->writeAttribute("Site geometric average <<O(|i-j|)>>_typ", "/fLBIT/model/lbits/corrtyp",
                                       "description");
                ...
                h5file->writeAttribute("The operator support matrix with shifted columns O(i,j) --> O(i,|i-j|)",
                                       "/fLBIT/model/lbits/corroff", "description");
                //                }
                ...
                h5file->writeAttribute(settings::flbit::cls::mpo_circuit_switchdepth, "/fLBIT/model/lbits",
                                       "mpo_switchdepth");
            }
        }
    ...
    if (num_rps > 0 and h5file and storage_event == StorageEvent::MODEL) {
        ...
        for (auto nrps: num::range(0, num_rps)) {
            ...
            tools::finite::mps::initialize_state(state_lbit_rps, StateInit::PRODUCT_STATE_NEEL_SHUFFLED,
                                                 StateInitType::REAL, "+z", false, 1, pattern);
            ...
            if (std::abs(rho_trace - static_cast<double>(length) / 2.0) > 1e-12)
                throw except::logic_error("R does not have trace L/2");
            if (eigvals.real().maxCoeff() > 1 + 1e-8)
                throw except::logic_error("The largest eigenvalue is larger than 1");
            if (eigvals.real().minCoeff() < 0 - 1e-8)
                throw except::logic_error("The smallest eigenvalue is smaller than 0");
        ...
        h5file->writeAttribute(
                "eigenvalues of the one-paricle densiy matrix: an exact step-function means non-interacting",
                "/fLBIT/model/opdm-eigv",
                "description");
...
void flbit::print_status(const AlgorithmStatus &st, const TensorsFinite &ts) {
    ...
    report +=
            fmt::format("χ:{:<3}|{:<3}|{:<3} ", settings::get_bond_max(st.algo_type), st.bond_lim,
                        tools::finite::measure::bond_dimension_midchain(*ts.state));
    if (settings::flbit::time_scale == TimeScale::LOGSPACED)
    ...
    report += fmt::format("mem[rss {:<.1f}|peak {:<.1f}|vm {:<.1f}]MB ", debug::mem_rss_in_mb(), debug::mem_hwm_in_mb(),
                          debug::mem_vm_in_mb());
