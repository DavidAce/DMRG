...
namespace pcg_extras {
    ...
    void generate_to_impl(SeedSeq &&generator, DestIter dest, std::false_type) {
        ...
        constexpr size_t FROM_ELEMS =
            GEN_IS_SMALLER ? size * ((DEST_SIZE + GEN_SIZE - 1) / GEN_SIZE) : (size + (GEN_SIZE / DEST_SIZE) - 1) / ((GEN_SIZE / DEST_SIZE) + GEN_IS_SMALLER);
    ...
    class seed_seq_from {
        ...
        constexpr size_t size() const {
            return (sizeof(typename RngType::result_type) > sizeof(result_type) && RngType::max() > ~size_t(0UL)) ? ~size_t(0UL) : size_t(RngType::max());
        }
    };
