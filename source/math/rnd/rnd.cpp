...
namespace rnd {
    ...
    ArrayXd random_with_replacement(const ArrayXd &in) {
        vector<double> boot;
        for(int i = 0; i < in.derived().size(); i++) { boot.push_back(in(uniform_integer_box(0, (int) (in.size() - 1)))); }
    ...
    ArrayXd random_with_replacement(const ArrayXd &in, const int n) {
        ...
        vector<double> boot;
        for(int i = 0; i < n; i++) { boot.push_back(in(uniform_integer_box(0, (int) (in.size() - 1)))); }
        return Eigen::Map<ArrayXd>(boot.data(), static_cast<Index>(boot.size()));
    }
