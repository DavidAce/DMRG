...
svd::config::config(std::optional<long> rank_max_, std::optional<double> truncation_lim_) : rank_max(rank_max_),
                                                                                            truncation_limit(
                                                                                                    truncation_lim_) {}
...
svd::solver::do_svd_ptr(const Scalar *mat_ptr, long rows, long cols,
    ...
    switch (svd_lib) {
        case svd::lib::lapacke: {
            try {
                ...
                try {
                    log->warn("{} {} failed to perform SVD: {} | Trying Lapacke gejsv", enum2sv(svd_lib),
                    ...
                    log->warn("{} {} failed to perform SVD: {} | Trying Eigen JacobiSVD", enum2sv(svd_lib),
                              enum2sv(svd_rtn), std::string_view(ex.what()));
                    auto svd_rtn_backup = svd_rtn; // Restore after
                    auto svd_log_level = log->level();
                    log->set_level(spdlog::level::trace);
        ...
        case svd::lib::eigen: {
            try {
                ...
                log->warn("{} {} failed to perform SVD: {} | Trying Eigen", enum2sv(svd_lib), enum2sv(svd_rtn),
...
template<typename Scalar>
void svd::solver::print_matrix([[maybe_unused]] const Scalar *mat_ptr, [[maybe_unused]] long rows,
                               [[maybe_unused]] long cols,
    ...
    for (long r = 0; r < A.rows(); r++) {
        if constexpr (std::is_same_v<Scalar, std::complex<double>>)
            for (long c = 0; c < A.cols(); c++)
                fmt::print("({1:.{0}f},{2:+.{0}f}) ", dec, std::real(A(r, c)), std::imag(A(r, c)));
        else
...
template<typename Scalar>
void svd::solver::print_vector([[maybe_unused]] const Scalar *vec_ptr, [[maybe_unused]] long size,
...
template void
svd::solver::print_matrix<real>(const real *vec_ptr, long rows, long cols, std::string_view tag, long dec) const;

template void
svd::solver::print_matrix<cplx>(const cplx *vec_ptr, long rows, long cols, std::string_view tag, long dec) const;
...
Eigen::Tensor<Scalar, 2> svd::solver::pseudo_inverse(const Eigen::Tensor<Scalar, 2> &tensor) {
    ...
    if (tensor.dimension(0) <= 0) {
        throw std::runtime_error("pseudo_inverse error: Dimension is zero: tensor.dimension(0)");
    }
    if (tensor.dimension(1) <= 0) {
        throw std::runtime_error("pseudo_inverse error: Dimension is zero: tensor.dimension(1)");
    }
...
std::pair<long, double> svd::solver::get_rank_from_truncation_error(const VectorType<double> &S) const {
    ...
    for (long s = 0; s <= S.size(); s++) {
        truncation_errors[s] = S.bottomRows(S.size() - s).norm();
    ...
    if (rank_min > 0)
        rank_ = std::max(rank_, std::min(S.size(),
    ...
    if (rank_ <= 0) {
        if (log)
            log->error(
                    "Size {} | Rank {} | Rank limit {} | truncation error limit {:8.2e} | error {:8.2e} truncation errors: {:8.2e}",
                    S.size(), rank_,
...
template<typename Scalar>
std::tuple<Eigen::Tensor<Scalar, 4>, Eigen::Tensor<Scalar, 2>>
svd::solver::split_mpo_l2r(const Eigen::Tensor<Scalar, 4> &mpo, const svd::config &svd_cfg) {
    /*
    ...
    auto avgS = num::next_power_of_two<double>(
            S.head(S.nonZeros()).mean()); // Nearest power of two larger than S.mean()
    if (avgS > 1) {
        S /= avgS;
        U *= avgS;
    }
...
template std::tuple<Eigen::Tensor<real, 4>, Eigen::Tensor<real, 2>>
svd::solver::split_mpo_l2r(const Eigen::Tensor<real, 4> &mpo, const svd::config &svd_cfg);

template std::tuple<Eigen::Tensor<cplx, 4>, Eigen::Tensor<cplx, 2>>
svd::solver::split_mpo_l2r(const Eigen::Tensor<cplx, 4> &mpo, const svd::config &svd_cfg);

template<typename Scalar>
std::tuple<Eigen::Tensor<Scalar, 2>, Eigen::Tensor<Scalar, 4>>
svd::solver::split_mpo_r2l(const Eigen::Tensor<Scalar, 4> &mpo, const svd::config &svd_cfg) {
    /*
    ...
    return std::make_tuple(
            ...
            tenx::TensorMap(V).reshape(tenx::array4{rank, dim1, dim2, dim3}).shuffle(
                    tenx::array4{0, 3, 1, 2}).template cast<Scalar>());
    /* clang-format on */
}

template std::tuple<Eigen::Tensor<real, 2>, Eigen::Tensor<real, 4>>
svd::solver::split_mpo_r2l(const Eigen::Tensor<real, 4> &mpo, const svd::config &svd_cfg);

template std::tuple<Eigen::Tensor<cplx, 2>, Eigen::Tensor<cplx, 4>>
svd::solver::split_mpo_r2l(const Eigen::Tensor<cplx, 4> &mpo, const svd::config &svd_cfg);
