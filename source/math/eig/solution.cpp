...
void eig::solution::build_eigvecs_cplx(){
    ...
    if(build_eigvecsR_cplx){
        ...
        for(size_t i = 0; i < eigvecsR_cplx.size(); i++){
            ...
            else if(not eigvecsR_real.empty() and i < eigvecsR_real.size()) eigvecsR_cplx[i] = std::complex<double>(eigvecsR_real[i],0.0);
            else if(not eigvecsR_imag.empty() and i < eigvecsR_imag.size()) eigvecsR_cplx[i] = std::complex<double>(0.0,eigvecsR_imag[i]);
        }
        eigvecsR_real.clear();
        eigvecsR_imag.clear();
    }
    if(build_eigvecsL_cplx){
        ...
        for(size_t i = 0; i < eigvecsL_cplx.size(); i++){
            ...
            else if(not eigvecsL_real.empty() and i < eigvecsL_real.size()) eigvecsL_cplx[i] = std::complex<double>(eigvecsL_real[i],0.0);
            else if(not eigvecsL_imag.empty() and i < eigvecsL_imag.size()) eigvecsL_cplx[i] = std::complex<double>(0.0,eigvecsL_imag[i]);
        }
        eigvecsL_real.clear();
        eigvecsL_imag.clear();
    }
