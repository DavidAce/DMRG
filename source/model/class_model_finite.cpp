...
class_mpo_base &class_model_finite::get_MPO(size_t pos) {
    return const_cast<class_mpo_base &>(static_cast<const class_model_finite &>(*this).get_MPO(pos));
}

size_t class_model_finite::get_length() const { return MPO.size(); }

bool class_model_finite::isReal() const {
    for(auto &mpo : MPO)
...
bool class_model_finite::hasNaN() const {
    for(auto &mpo : MPO)
        if(mpo->hasNaN()) return true;
    return false;
}

void class_model_finite::assertValidity() const {
    for(auto &mpo : MPO) mpo->assertValidity();
}

// For reduced energy MPO's

bool class_model_finite::isReduced() const {
...
const Eigen::Tensor<class_model_finite::Scalar, 4> &class_model_finite::get_multisite_mpo() const{
    return tools::finite::views::get_multisite_mpo(*this);
}
