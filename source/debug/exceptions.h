...
namespace except {
    ...
    [[nodiscard]] std::runtime_error runtime_error(std::string_view s, Args... args) {
        if (s.rfind("dmrg++: ", 0) == 0)  return std::runtime_error(fmt::format(s, std::forward<Args>(args)...));
        else
    ...
    [[nodiscard]] std::logic_error logic_error(std::string_view s, Args... args) {
        if (s.rfind("dmrg++: ", 0) == 0)  return std::logic_error(fmt::format(s, std::forward<Args>(args)...));
        else
