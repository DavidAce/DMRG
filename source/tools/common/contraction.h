...
namespace tools::common::contraction {
    ...
    using real = double;
    template<typename T> using TensorWrite = Eigen::TensorBase<T,Eigen::WriteAccessors>;
    template<typename T> using TensorRead = Eigen::TensorBase<T,Eigen::ReadOnlyAccessors>;
    ...
    void contract_mps_bnd(
        ...
        contract_mps_bnd(res_ref.data(), res_ref.dimensions(),
                         mps_ref.data(), mps_ref.dimensions(),
                         bnd_ref.data(), bnd_ref.dimensions());
    }
    ...
    void contract_bnd_mps(
        ...
        const auto & bnd_ref  = static_cast<const bnd_type&>(bnd);
        res_ref.resize(mps_ref.dimensions());
        contract_bnd_mps(res_ref.data(), res_ref.dimensions(),
                         bnd_ref.data(), bnd_ref.dimensions(),
                         mps_ref.data(), mps_ref.dimensions());
    }
    ...
    [[nodiscard]] Eigen::Tensor<typename mps_type::Scalar, 3> contract_mps_bnd_temp(
        const TensorRead<mps_type> & mps,
        const TensorRead<bnd_type> & bnd,
    ...
    [[nodiscard]] Eigen::Tensor<typename mps_type::Scalar, 3> contract_bnd_mps_temp(
        const TensorRead<bnd_type> & bnd,
        const TensorRead<mps_type> & mps,
        Eigen::Tensor<typename mps_type::Scalar, 3> & temp){
    ...
    void contract_mps_mps(
        TensorWrite<mps_type> &res,
        const TensorRead<mps_type> & mpsL,
        const TensorRead<mps_type> & mpsR){
        static_assert(mps_type::NumIndices == 3 and "Wrong mps tensor rank != 3");
        ...
        contract_mps_mps(res_ref.data(), res_ref.dimensions(),
                         mpsL_ref.data(), mpsL_ref.dimensions(),
                         mpsR_ref.data(), mpsR_ref.dimensions());
    }

    template<typename mps_type>
    [[nodiscard]] Eigen::Tensor<typename mps_type::Scalar, 3> contract_mps_mps_temp(
        const TensorRead<mps_type> & mpsL,
        const TensorRead<mps_type> & mpsR,
        Eigen::Tensor<typename mps_type::Scalar, 3> & temp){
    ...
    template<typename Scalar>
    double contract_mps_mps_overlap(const Scalar * const mps1_ptr, std::array<long,3> mps1_dims,
    ...
    double contract_mps_norm(const TensorRead<mps_type> & mps1, const TensorRead<mps_type> & mps2 ){
        ...
        return contract_mps_overlap(mps1_ref.data(), mps1_ref.dimensions(),
                                    mps2_ref.data(), mps2_ref.dimensions());
    }
    ...
    double contract_mps_norm(const TensorRead<mps_type> & mps){
        static_assert(mps_type::NumIndices == 3 and "Wrong mps tensor rank != 3");
        const auto & mps_ref  = static_cast<const mps_type&>(mps);
        return contract_mps_mps_overlap(mps_ref.data(), mps_ref.dimensions(),
                                        mps_ref.data(), mps_ref.dimensions());
    }
    ...
    template<typename mps_type>
    [[nodiscard]] Eigen::Tensor<typename mps_type::Scalar, 2> contract_mps_mps_partial(const TensorRead<mps_type> & mps1,const TensorRead<mps_type> & mps2 ,std::array<long,2> idx){
        static_assert(mps_type::NumIndices == 3 and "Wrong mps tensor rank != 3");
        const auto & mps1_ref  = static_cast<const mps_type&>(mps1);
        const auto & mps2_ref  = static_cast<const mps_type&>(mps2);
        std::array<long,2> dims =  {};
        if (idx[1] == 1) dims = {mps1_ref.dimension(2), mps2_ref.dimension(2)};
        else if (idx[1] == 2) dims = {mps1_ref.dimension(1), mps2_ref.dimension(1)};
        else throw except::runtime_error("Expected idx [0,1] or [0,2]. Got: {}", idx);
        Eigen::Tensor<typename mps_type::Scalar,2> res(dims);
        contract_mps_mps_partial(res.data(), res.dimensions(),
                                 mps1_ref.data(), mps1_ref.dimensions(),
                                 mps2_ref.data(), mps2_ref.dimensions(),
                                 idx);
        return res;
    }
    ...
    [[nodiscard]] Eigen::Tensor<typename mps_type::Scalar, 2> contract_mps_partial(const TensorRead<mps_type> & mps, std::array<long,2> idx){
        ...
        const auto & mps_ref  = static_cast<const mps_type&>(mps);
        std::array<long,2> dims =  {};
        if (idx[1] == 1) dims = {mps_ref.dimension(2), mps_ref.dimension(2)};
        else if (idx[1] == 2) dims = {mps_ref.dimension(1), mps_ref.dimension(1)};
        else throw except::runtime_error("Expected idx [0,1] or [0,2]. Got: {}", idx);
        Eigen::Tensor<typename mps_type::Scalar,2> res(dims);
        contract_mps_mps_partial(res.data(), res.dimensions(),
                                        mps_ref.data(), mps_ref.dimensions(),
