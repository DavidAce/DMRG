...
namespace tools::common::h5 {
    ...
    void save::bootstrap_save_log(std::unordered_map<std::string, std::pair<uint64_t, uint64_t>> &save_log, const h5pp::File &h5ppFile, std::string_view link) {
        // from h5table
        if(save_log.empty()) {
            try {
    ...
    void save::status(h5pp::File &h5ppFile, std::string_view table_prefix, const StorageLevel &storage_level, const AlgorithmStatus &status) {
    ...
    void save::mem(h5pp::File &h5ppFile, std::string_view table_prefix, const StorageLevel &storage_level, const AlgorithmStatus &status) {
        if(storage_level == StorageLevel::NONE) return;
        // Check if the current entry has already been appended
    ...
    void save::meta(h5pp::File &h5ppFile, const StorageLevel &storage_level, const StorageReason &storage_reason, const ModelType &model_type,
        ...
        save::attr(h5ppFile, storage_level_sv, state_prefix, "common/storage_level", "Maps state_prefix -> storage_level");
        save::attr(h5ppFile, storage_reason_sv, state_prefix, "common/storage_reason", "Maps state_prefix -> storage_reason");
        ...
        save::attr(h5ppFile, model_name_sv, state_prefix, "common/model_type", "Maps state_prefix -> model_type");
        save::attr(h5ppFile, model_size, state_prefix, "common/model_size", "Maps state_prefix -> model_size");
        save::attr(h5ppFile, status.algo_type_sv(), state_prefix, "common/algo_type", "Maps state_prefix -> algo_type");
    ...
    void save::timer(h5pp::File &h5ppFile, std::string_view state_prefix, const StorageLevel &storage_level, const AlgorithmStatus &status) {
