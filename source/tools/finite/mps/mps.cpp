...
void tools::finite::mps::apply_gate(StateFinite &state, const qm::Gate &gate, Eigen::Tensor<cplx, 3> &temp, bool reverse, long chi_lim, GateMove gm,
    ...
    if(gm == GateMove::ON) {
        ...
        auto steps            = move_center_point_to_pos_dir(state, move_to_position, 1, chi_lim, svd_settings);
    ...
    tools::finite::mps::merge_multisite_mps(state, temp, gate.pos, center_position, chi_lim, svd_settings, LogPolicy::QUIET);
}
...
void tools::finite::mps::apply_gates(StateFinite &state, const std::vector<Eigen::Tensor<cplx, 2>> &nsite_tensors, size_t gate_size, bool reverse, long chi_lim,
                                     GateMove gm, std::optional<svd::settings> svd_settings) {
    ...
    apply_gates(state, gates, reverse, chi_lim, gm, svd_settings);
}

void tools::finite::mps::apply_gates(StateFinite &state, const std::vector<qm::Gate> &gates, bool reverse, long chi_lim, GateMove gm,
                                     std::optional<svd::settings> svd_settings) {
    ...
    for(const auto &idx : gate_sequence) { apply_gate(state, gates.at(idx), gate_mps, reverse, chi_lim, gm, svd_settings); }
...
void tools::finite::mps::apply_swap_gate(StateFinite &state, qm::SwapGate &gate, Eigen::Tensor<cplx, 3> &temp, bool reverse, long chi_lim,
    ...
    tools::finite::mps::merge_multisite_mps(state, temp, pos, center_position, chi_lim, svd_settings, LogPolicy::QUIET);
...
void tools::finite::mps::apply_swap_gates(StateFinite &state, std::vector<qm::SwapGate> &gates, bool reverse, long chi_lim, GateMove gm,
                                          std::optional<svd::settings> svd_settings) {
    ...
    for(const auto &[i, gate_idx] : iter::enumerate(gate_sequence)) {
        ...
        apply_swap_gate(state, gate, temp, reverse, chi_lim, order, gm, svd_settings);
    }
    move_center_point_to_pos_dir(state, 0, 1, chi_lim, svd_settings);
    tools::finite::mps::normalize_state(state, chi_lim, svd_settings, NormPolicy::IFNEEDED);
