...
void tools::finite::io::h5table::write_measurements(h5pp::File &h5ppFile, const std::string &table_prefix, const StorageLevel &storage_level,
    ...
    measurement_entry.step                            = static_cast<uint64_t>(sim_status.step);
    measurement_entry.iter                            = static_cast<uint64_t>(sim_status.iter);
    measurement_entry.position                        = static_cast<uint64_t>(sim_status.position);
    ...
    measurement_entry.energy_variance_per_site_lowest = state.lowest_recorded_variance / static_cast<double>(state.get_length());
