...
namespace tools::finite::ops {
    ...
    [[nodiscard]] extern std::optional<double> get_spin_component_in_sector     (StateFinite & state, std::string_view sector);
                  extern void                  project_to_sector                (StateFinite & state, const Eigen::MatrixXcd & paulimatrix, int sign, std::optional<svd::config> svd_cfg = std::nullopt);
    [[nodiscard]] extern int                   project_to_nearest_sector        (StateFinite & state, std::string_view sector, std::optional<svd::config> svd_cfg = std::nullopt);
    [[nodiscard]] extern StateFinite           get_projection_to_sector         (const StateFinite & state, const Eigen::MatrixXcd & paulimatrix, int sign, std::optional<svd::config> svd_cfg = std::nullopt);
    [[nodiscard]] extern StateFinite           get_projection_to_nearest_sector (const StateFinite & state, std::string_view  sector, std::optional<svd::config> svd_cfg = std::nullopt);
