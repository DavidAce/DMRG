...
namespace tools::finite::multisite {
    extern Eigen::DSizes<long, 3> get_dimensions(const class_state_finite &state, std::optional<std::vector<size_t>> sites = std::nullopt);
    extern Eigen::DSizes<long, 4> get_dimensions(const class_model_finite &model, std::optional<std::vector<size_t>> sites = std::nullopt);
