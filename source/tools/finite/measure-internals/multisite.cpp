...
double tools::finite::measure::multisite::energy_minus_energy_reduced(const state_or_mps_type &state, const class_model_finite &model,
    ...
    if constexpr(std::is_same_v<state_or_mps_type, class_state_finite>) {
        if(not math::all_equal(state.active_sites, model.active_sites, edges.active_sites))
            throw std::runtime_error(fmt::format("Could not compute energy: active sites are not equal: state {} | model {} | edges {}",
                                                 state.active_sites, model.active_sites, edges.active_sites));
        return tools::finite::measure::multisite::energy_minus_energy_reduced(state.get_multisite_mps(), model, edges);
    } else {
        const auto &mpo = model.get_multisite_mpo();
        const auto &env = edges.get_multisite_ene();
        tools::common::profile::t_ene->tic();
        double e_minus_ered = tools::finite::measure::multisite::moments::first(state, mpo, env.first.get(), env.second.get());
        tools::common::profile::t_ene->toc();
        return e_minus_ered;
    }
}

template double tools::finite::measure::multisite::energy_minus_energy_reduced(const class_state_finite &, const class_model_finite &model,
                                                                               const class_edges_finite &edges);
template double tools::finite::measure::multisite::energy_minus_energy_reduced(const Eigen::Tensor<Scalar, 3> &, const class_model_finite &model,
                                                                               const class_edges_finite &edges);


template<typename state_or_mps_type>
double tools::finite::measure::multisite::energy(const state_or_mps_type &state, const class_model_finite &model, const class_edges_finite &edges) {
    ...
    if constexpr (std::is_same_v<state_or_mps_type, class_state_finite>){
        if(state.measurements.energy) return state.measurements.energy.value();
            ...
            double energy = tools::finite::measure::multisite::energy_minus_energy_reduced(state, model, edges) + model.get_energy_reduced();
            state.measurements.energy = tools::finite::measure::multisite::energy(state.get_multisite_mps(), model, edges);
            return state.measurements.energy.value();
        }
    }else{
        return tools::finite::measure::multisite::energy_minus_energy_reduced(state, model, edges) + model.get_energy_reduced();
    }
}


template double tools::finite::measure::multisite::energy(const class_state_finite &, const class_model_finite &model,
                                                                               const class_edges_finite &edges);
template double tools::finite::measure::multisite::energy(const Eigen::Tensor<Scalar, 3> &, const class_model_finite &model,
                                                                               const class_edges_finite &edges);

template<typename state_or_mps_type>
double tools::finite::measure::multisite::energy_per_site(const state_or_mps_type &state, const class_model_finite &model, const class_edges_finite &edges) {
    return tools::finite::measure::multisite::energy(state, model, edges) / static_cast<double>(model.get_length());
}


template double tools::finite::measure::multisite::energy_per_site(const class_state_finite &, const class_model_finite &model,
                                                          const class_edges_finite &edges);
template double tools::finite::measure::multisite::energy_per_site(const Eigen::Tensor<Scalar, 3> &, const class_model_finite &model,
                                                          const class_edges_finite &edges);

template<typename state_or_mps_type>
double tools::finite::measure::multisite::energy_variance(const state_or_mps_type &state, const class_model_finite &model, const class_edges_finite &edges) {
    ...
    if constexpr(std::is_same_v<state_or_mps_type, class_state_finite>){
        if(not math::all_equal(state.active_sites, model.active_sites, edges.active_sites))
            throw std::runtime_error(fmt::format("Could not compute energy variance: active sites are not equal: state {} | model {} | edges {}", state.active_sites,
        ...
        auto var = tools::finite::measure::multisite::energy_variance(state.get_multisite_mps(), model, edges);
        if(var < state.lowest_recorded_variance) state.lowest_recorded_variance = var;
        state.measurements.energy_variance = var;
        return var;
    }
    else{
        double energy = 0;
        if(model.isReduced())
            energy = multisite::energy_minus_energy_reduced(state, model, edges);
        else
            energy = multisite::energy(state, model, edges);
        ...
        if(not math::all_equal(model.active_sites, edges.active_sites))
            throw std::runtime_error(fmt::format("Could not compute energy variance: active sites are not equal: model {} | edges {}",
                                                 model.active_sites, edges.active_sites));
        const auto &mpo = model.get_multisite_mpo();
        const auto &env = edges.get_multisite_var();
        tools::common::profile::t_ene->tic();
        double H2 = tools::finite::measure::multisite::moments::second(state, mpo, env.first.get(), env.second.get());
        tools::common::profile::t_ene->toc();
...
template double tools::finite::measure::multisite::energy_variance(const class_state_finite &, const class_model_finite &model,
                                                                   const class_edges_finite &edges);
template double tools::finite::measure::multisite::energy_variance(const Eigen::Tensor<Scalar, 3> &, const class_model_finite &model,
                                                                   const class_edges_finite &edges);


template<typename state_or_mps_type>
double tools::finite::measure::multisite::energy_variance_per_site(const state_or_mps_type &state, const class_model_finite &model,
                                                                   const class_edges_finite &edges) {
    return multisite::energy_variance(state, model,edges) / static_cast<double>(model.get_length());
}

template double tools::finite::measure::multisite::energy_variance_per_site(const class_state_finite &, const class_model_finite &model,
                                                                   const class_edges_finite &edges);
template double tools::finite::measure::multisite::energy_variance_per_site(const Eigen::Tensor<Scalar, 3> &, const class_model_finite &model,
                                                                   const class_edges_finite &edges);
