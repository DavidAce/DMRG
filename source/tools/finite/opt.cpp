...
tools::finite::opt::opt_mps tools::finite::opt::find_excited_state(const class_tensors_finite &tensors, const opt_mps &initial_tensor,
    ...
    ceres_default_options.max_num_line_search_direction_restarts     = 5; //5
    ...
    ceres_default_options.gradient_tolerance                         = 1e-3; // Not tested yet
    ...
    if(status.algorithm_has_got_stuck){
        ...
        ceres_default_options.parameter_tolerance                       = 5e-12;
    ...
    if(status.algorithm_has_stuck_for > 10){
