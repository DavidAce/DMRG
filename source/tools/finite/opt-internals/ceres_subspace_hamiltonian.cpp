...
Eigen::Tensor<std::complex<double>, 6> tools::finite::opt::internal::local_hamiltonians::get_multi_hamiltonian_squared_tensor(const class_model_finite &model,
                                                                                                                              const class_edges_finite &edges) {
    const auto &mpo = model.get_multisite_tensor();
...
Eigen::MatrixXcd tools::finite::opt::internal::local_hamiltonians::get_multi_hamiltonian_squared_subspace_matrix(const class_model_finite &model,
                                                                                                                 const class_edges_finite &edges,
                                                                                                                 const Eigen::MatrixXcd &  eigvecs) {
    ...
    if(log2spin >= std::max(log2chiL, log2chiR)) {
        if(log2chiL >= log2chiR) {
            tools::log->trace("get_H2 path: log2spin >= std::max(log2chiL, log2chiR)  and  log2chiL >= log2chiR ");
            H2 = eigvecs_tensor.contract(env.L, Textra::idx({1}, {0}))
            ...
            tools::log->trace("get_H2 path: log2spin >= std::max(log2chiL, log2chiR)  and  log2chiL < log2chiR ");
            H2 = eigvecs_tensor.contract(env.R, Textra::idx({2}, {0}))
        ...
        H2 =
            ...
            eigvecs_tensor.contract(env.L, Textra::idx({1}, {0}))
                .contract(mpo, Textra::idx({0, 4}, {2, 0}))
...
Eigen::Tensor<T, 6> tools::finite::opt::internal::get_multi_hamiltonian_squared_tensor(const class_model_finite &model, const class_edges_finite &edges) {
    static_assert(std::is_same<T, std::complex<double>>::value or std::is_same<T, double>::value, "Wrong type, expected double or complex double");
    if constexpr(std::is_same<T, std::complex<double>>::value)
        return local_hamiltonians::get_multi_hamiltonian_squared_tensor(model, edges);
    else if constexpr(std::is_same<T, double>::value)
...
MatrixType<T> tools::finite::opt::internal::get_multi_hamiltonian_matrix(const class_model_finite &model, const class_edges_finite &edges) {
    static_assert(std::is_same<T, std::complex<double>>::value or std::is_same<T, double>::value, "Wrong type, expected double or complex double");
    if constexpr(std::is_same<T, std::complex<double>>::value)
        return local_hamiltonians::get_multi_hamiltonian_matrix(model, edges);
    else if constexpr(std::is_same<T, double>::value)
        return local_hamiltonians::get_multi_hamiltonian_matrix(model, edges).real();
}
// Explicit instantiations
template MatrixType<double> tools::finite::opt::internal::get_multi_hamiltonian_matrix<double>(const class_model_finite &model,
                                                                                               const class_edges_finite &edges);
template MatrixType<Scalar> tools::finite::opt::internal::get_multi_hamiltonian_matrix<Scalar>(const class_model_finite &model,
                                                                                               const class_edges_finite &edges);
...
MatrixType<T> tools::finite::opt::internal::get_multi_hamiltonian_squared_matrix(const class_model_finite &model, const class_edges_finite &edges) {
    static_assert(std::is_same<T, std::complex<double>>::value or std::is_same<T, double>::value, "Wrong type, expected double or complex double");
    if constexpr(std::is_same<T, std::complex<double>>::value)
        return local_hamiltonians::get_multi_hamiltonian_squared_matrix(model, edges);
    else if constexpr(std::is_same<T, double>::value)
...
template<typename T>
MatrixType<T> tools::finite::opt::internal::get_multi_hamiltonian_squared_subspace_matrix(const class_model_finite &model, const class_edges_finite &edges,
                                                                                          const Eigen::MatrixXcd &eigvecs) {
    static_assert(std::is_same<T, std::complex<double>>::value or std::is_same<T, double>::value, "Wrong type, expected double or complex double");
    if constexpr(std::is_same<T, std::complex<double>>::value)
        return local_hamiltonians::get_multi_hamiltonian_squared_subspace_matrix(model, edges, eigvecs);
    else if constexpr(std::is_same<T, double>::value)
        return local_hamiltonians::get_multi_hamiltonian_squared_subspace_matrix(model, edges, eigvecs).real();
}
// Explicit instantiations
template MatrixType<double> tools::finite::opt::internal::get_multi_hamiltonian_squared_subspace_matrix<double>(const class_model_finite &model,
                                                                                                                const class_edges_finite &edges,
                                                                                                                const Eigen::MatrixXcd &  eigvecs);
template MatrixType<Scalar> tools::finite::opt::internal::get_multi_hamiltonian_squared_subspace_matrix<Scalar>(const class_model_finite &model,
                                                                                                                const class_edges_finite &edges,
                                                                                                                const Eigen::MatrixXcd &  eigvecs);
...
MatrixType<T> tools::finite::opt::internal::get_multi_hamiltonian_squared_subspace_matrix_new(const class_model_finite &model, const class_edges_finite &edges,
    ...
    static_assert(std::is_same<T, std::complex<double>>::value or std::is_same<T, double>::value, "Wrong type, expected double or complex double");
    if constexpr(std::is_same<T, std::complex<double>>::value)
        return local_hamiltonians::get_multi_hamiltonian_squared_subspace_matrix_new(model, edges, eigvecs);
    else if constexpr(std::is_same<T, double>::value)
        return local_hamiltonians::get_multi_hamiltonian_squared_subspace_matrix_new(model, edges, eigvecs).real();
}
// Explicit instantiations
