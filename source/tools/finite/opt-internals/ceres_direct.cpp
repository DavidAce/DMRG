...
tools::finite::opt::internal::ceres_direct_optimization(const class_state_finite &state,
    ...
    if (tools::log->level() <= spdlog::level::debug and optSpace == OptSpace::DIRECT){
        ...
        t_local.toc();
        reports::bfgs_log.emplace_back("Current state" ,theta_old.size(),0, energy_old, std::log10(variance_old), 1.0, theta_old_vec.norm(), 1,1, t_local.get_last_time_interval());
    }
    if (tools::log->level() <= spdlog::level::trace and optSpace == OptSpace::DIRECT and settings::debug){
        ...
        t_local.toc();
        reports::bfgs_log.emplace_back("Initial guess" , theta_initial.size(),0, energy_initial, std::log10(variance_initial), 1.0, theta_initial_vec.norm(), 1,1, t_local.get_last_time_interval());
    }
    ...
    for(auto & bfgs_tol : bfgs_tols){
        ...
        reports::bfgs_log.emplace_back("LBFGS direct", theta_new.size(), options.max_lbfgs_rank, energy_ceres, std::log10(variance_ceres), overlap_new, theta_new.norm(), iter, counter,t_local.get_last_time_interval());
        ...
        reports::time_log.emplace_back(
            tools::common::profile::t_vH2v->get_measured_time_and_reset(),
