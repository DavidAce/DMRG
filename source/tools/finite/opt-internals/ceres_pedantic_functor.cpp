...
ceres_pedantic_functor<Scalar>::ceres_pedantic_functor(
...
template<typename Scalar>
bool ceres_pedantic_functor<Scalar>::Evaluate(const double* v_double_double,
                                            double* fx,
...
template<typename Scalar>
void ceres_pedantic_functor<Scalar>::get_H2v (const VectorType &v)const{
    t_vH2->tic();
...
template<typename Scalar>
void ceres_pedantic_functor<Scalar>::get_Hv (const VectorType &v)const{
    t_vH->tic();
    size_t log2chiL  = std::log2(dsizes[1]);
    size_t log2chiR  = std::log2(dsizes[2]);
...
//template<typename Scalar>
//std::pair<typename ceres_pedantic_functor<Scalar>::VectorType, Scalar>
//ceres_pedantic_functor<Scalar>::get_Hv_vHv(const VectorType &v)const{
//    auto Hv = get_Hv(v);
//    t_vHv->tic();
//    auto vHv = v.dot(Hv);
//    t_vHv->toc();
...
//template<typename Scalar>
//std::pair<typename ceres_pedantic_functor<Scalar>::VectorType, Scalar>
//ceres_pedantic_functor<Scalar>::get_H2v_vH2v(const VectorType &v)const{
//    auto H2v = get_H2v(v);
//
...
template class tools::finite::opt::internal::ceres_pedantic_functor<double>;
template class tools::finite::opt::internal::ceres_pedantic_functor<std::complex<double>>;
