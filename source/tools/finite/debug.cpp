...
void tools::finite::debug::check_integrity(const class_state_finite &state) {
    ...
    //            throw std::runtime_error(fmt::format("Mismatch in ENV_R size+1 and length-position: {} != {}", state.ENV_R.front().sites, state.get_length() -
    //            state.get_position() - 2));
    //
    ...
    //                    throw std::runtime_error(fmt::format("Center and left dimension mismatch {} != {}", state.MPS_L.back().get_chiR(),
    //                    state.MPS_L.back().get_LC().dimension(0)));
    //                if(state.MPS_R.front().get_chiL() != state.MPS_L.back().get_LC().dimension(0) )
    //                    throw std::runtime_error(fmt::format("Center and right dimension mismatch {} != {}", state.MPS_L.back().get_chiR(),
    //                    state.MPS_L.back().get_LC().dimension(0)));
    //            }
    //        }
    //
    ...
    //                throw std::runtime_error(fmt::format("Mismatch in MPS and MPO positions @ pos {}: {} != {}", pos, mps_here.get_position(),
    //                mpo_here.get_position()));
    //
    //            if(mps_here.get_position() - mps_left.get_position() != 1)
    //                throw std::runtime_error(fmt::format("Mismatch in adjacent MPS positions @ pos {}: {} - {} != 1", pos, mps_here.get_position() ,
    //                mps_left.get_position()));
    //
    //            if(mps_left.get_chiR() != mps_here.get_chiL())
    //                throw std::runtime_error(fmt::format("Mismatch in adjacent MPS dimensions @ pos {}: {} != {}", pos, mps_left.get_chiR() ,
    //                mps_here.get_chiL()));
    //
    //
    //            if(mpo_left.MPO().dimension(1) != mpo_here.MPO().dimension(0))
    //                throw std::runtime_error(fmt::format("Mismatch in adjacent MPO dimensions @ pos {}: {} != {}", pos, mpo_left.MPO().dimension(1)  ,
    //                mpo_here.MPO().dimension(0)));
    //
    //            if(mps_here.spin_dim() != mpo_here.MPO().dimension(2))
    //                throw std::runtime_error(fmt::format("Mismatch in MPS and MPO spin dimensions @ pos {}: {} != {}", pos, mps_here.spin_dim() ,
    //                mpo_here.MPO().dimension(2)));
    //        }
    //
    //        {
    ...
    //                    throw std::runtime_error(fmt::format("Mismatch in MPS and ENV positions (left side) @ site {}: {} != {}", i, mps_it->get_position(),
    //                    env_it->get_position()));
    //
    //                if(mps_it->get_position() != env_it->sites)
    //                    throw std::runtime_error(fmt::format("Mismatch in MPS position and ENV size (left side) @ site {}: {} != {}", i,
    //                    mps_it->get_position(), env_it->sites));
    //
    //                if(mps_it->get_chiL() != env_it->block.dimension(0))
    //                    throw std::runtime_error(fmt::format("Mismatch in MPS and ENV dimensions (left side) @ site {}: {} != {}", i,mps_it->get_chiL() ,
    //                    env_it->block.dimension(0)));
    //
    //                if(env_nx->get_position() - env_it->get_position() != 1)
    //                    throw std::runtime_error(fmt::format("Mismatch in adjacent ENV positions (left side) @ site {}: {} - {} != 1", i,
    //                    env_nx->get_position(), env_it->get_position()));
    //
    //                if(env_it->block.dimension(2) != mpo_it->get()->MPO().dimension(0))
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV and MPO dimensions (left side) @ site {}: {} != {}",
    //                    i,env_it->block.dimension(2), mpo_it->get()->MPO().dimension(0)));
    //
    //
    //                if(env2_it->block.dimension(2) != mpo_it->get()->MPO().dimension(0))
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 and MPO dimensions (left side) @ site {}: {} != {}",
    //                    i,env2_it->block.dimension(2), mpo_it->get()->MPO().dimension(0)));
    //
    //                if(env2_it->block.dimension(3) != mpo_it->get()->MPO().dimension(0))
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 and MPO dimensions (left side) @ site {}: {} != {}",
    //                    i,env2_it->block.dimension(3) ,mpo_it->get()->MPO().dimension(0)));
    //
    //
    //                if(env2_it->sites != env_it->sites)
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 position and ENV sites (left side) @ site {}: {} != {}", i,env2_it->sites !=
    //                    env_it->sites));
    //
    //                if(env2_it->get_position() != env_it->get_position())
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 position and ENV positions (left side) @ site {}: {} != {}",
    //                    i,env2_it->get_position(), env_it->get_position()));
    //
    //                mps_it++;
    //                mps_nx++;
    //                env_it++;
    ...
    //                    throw std::runtime_error(fmt::format("Mismatch in adjacent MPS dimensions (right side) @ site {}: {} != {}", i,  mps_nx->get_chiR(),
    //                    mps_it->get_chiL()));
    //
    //                if(mps_it->get_position() - mps_nx->get_position() != 1 )
    //                    throw std::runtime_error(fmt::format("Mismatch in adjacent MPS positions (right side) @ site {}: {} - {} != 1", i,
    //                    mps_it->get_position(), mps_nx->get_position()));
    //
    //                if(mps_it->get_position() != env_it->get_position())
    //                    throw std::runtime_error(fmt::format("Mismatch in MPS and ENV positions (right side) @ site {}: {} != {}", i,  mps_it->get_position(),
    //                    env_it->get_position()));
    //
    //                if(mps_it->get_position() != state.get_length() - (env_it->sites + 1))
    //                    throw std::runtime_error(fmt::format("Mismatch in MPS position and ENV size + 1 (right side) @ site {}: {} != {}", i,
    //                    mps_it->get_position(), state.get_length() - (env_it->sites + 1)));
    //
    //                if(mps_it->get_position() != env_it->get_position())
    //                    throw std::runtime_error(fmt::format("Mismatch in MPS position and ENV position (right side) @ site {}: {} != {}", i,
    //                    mps_it->get_position(), env_it->get_position()));
    //
    //                if(mps_it->get_chiR() != env_it->block.dimension(0))
    //                    throw std::runtime_error(fmt::format("Mismatch in MPS and ENV dimensions (right side) @ site {}: {} != {}", i,  mps_it->get_chiR(),
    //                    env_it->block.dimension(0)));
    //
    //                if(env_it->get_position() - env_nx->get_position() != 1)
    //                    throw std::runtime_error(fmt::format("Mismatch in adjacent ENV positions (right side) @ site {}: {} - {} != 1", i,
    //                    env_it->get_position(), env_nx->get_position()));
    //
    //                if(env2_it->sites != env_it->sites)
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 position and ENV sites (right side) @ site {}: {} != {}", i,  env2_it->sites,
    //                    env_it->sites));
    //
    //                if(env2_it->get_position() != env_it->get_position())
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 position and ENV positions (right side) @ site {}: {} != {}", i,
    //                    env2_it->get_position(), env_it->get_position()));
    //
    //
    //                if(env_it->block.dimension(2) != mpo_it->get()->MPO().dimension(1))
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV and MPO dimensions (right side) @ site {}: {} != {}", i,
    //                    env_it->block.dimension(2), mpo_it->get()->MPO().dimension(1)));
    //
    //                if(env2_it->block.dimension(2) != mpo_it->get()->MPO().dimension(1))
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 and MPO dimensions (right side) @ site {}: {} != {}", i,
    //                    env2_it->block.dimension(2), mpo_it->get()->MPO().dimension(1)));
    //
    //                if(env2_it->block.dimension(3) != mpo_it->get()->MPO().dimension(1))
    //                    throw std::runtime_error(fmt::format("Mismatch in ENV2 and MPO dimensions (right side) @ site {}: {} != {}", i,
    //                    env2_it->block.dimension(3), mpo_it->get()->MPO().dimension(1)));
    //
    //
    //                mps_it++;
    //                mps_nx++;
...
void tools::finite::debug::check_integrity(const class_model_finite &model) {
    ...
    //            if (not mpo->all_mpo_parameters_have_been_set){throw std::runtime_error(fmt::format("All parameters have not been set on MPO_L site: {}"
    //            ,site));}
    //        }
    //        for (auto &mpo : state.MPO_R){
    //            auto site = mpo->get_position();
    //            if (not mpo->all_mpo_parameters_have_been_set){throw std::runtime_error(fmt::format("All parameters have not been set on MPO_R site: {}"
    //            ,site));}
    //        }
    //    }
    //    catch(std::exception &ex){
...
void tools::finite::debug::check_integrity(const class_edges_finite &edges) {
    ...
    //            throw std::runtime_error(fmt::format("Lengths not all equal: state {} | model {} | edges {}", state.get_length(),model.get_length(),
    //            edges.get_length()));
...
void tools::finite::debug::check_integrity(const class_state_finite &state, const class_model_finite &model, const class_edges_finite &edges) {
    ...
    //        throw std::runtime_error(fmt::format("Active sites not all equal: state {} | model {} | edges {}", state.active_sites,model.active_sites,
    //        edges.active_sites));
    //
    //    if(not num::all_equal(state.active_sites,model.active_sites, edges.active_sites))
    //        throw std::runtime_error(fmt::format("Active sites not all equal: state {} | model {} | edges {}", state.active_sites,model.active_sites,
    ...
    //
    //            if(mps_it->get_position() != env_it->sites)
    //                throw std::runtime_error(fmt::format("Mismatch in MPS position and ENV size (left side) @ site {}: {} != {}", i, mps_it->get_position(),
    //                env_it->sites));
    //
    //            if(mps_it->get_chiL() != env_it->block.dimension(0))
    //                throw std::runtime_error(fmt::format("Mismatch in MPS and ENV dimensions (left side) @ site {}: {} != {}", i,mps_it->get_chiL() ,
    ...
    //
    //            if(env_nx->get_position() - env_it->get_position() != 1)
    //                throw std::runtime_error(fmt::format("Mismatch in adjacent ENV positions (left side) @ site {}: {} - {} != 1", i, env_nx->get_position(),
    //                env_it->get_position()));
    //
    //            if(env_it->block.dimension(2) != mpo_it->get()->MPO().dimension(0))
    //                throw std::runtime_error(fmt::format("Mismatch in ENV and MPO dimensions (left side) @ site {}: {} != {}", i,env_it->block.dimension(2),
    //                mpo_it->get()->MPO().dimension(0)));
    //
    //
    //            if(env2_it->block.dimension(2) != mpo_it->get()->MPO().dimension(0))
    //                throw std::runtime_error(fmt::format("Mismatch in ENV2 and MPO dimensions (left side) @ site {}: {} != {}", i,env2_it->block.dimension(2),
    //                mpo_it->get()->MPO().dimension(0)));
    //
    //            if(env2_it->block.dimension(3) != mpo_it->get()->MPO().dimension(0))
    //                throw std::runtime_error(fmt::format("Mismatch in ENV2 and MPO dimensions (left side) @ site {}: {} != {}", i,env2_it->block.dimension(3)
    //                ,mpo_it->get()->MPO().dimension(0)));
    //
    //
    //            if(env2_it->sites != env_it->sites)
    ...
    //
    //            if(mps_it->get_position() - mps_nx->get_position() != 1 )
    //                throw std::runtime_error(fmt::format("Mismatch in adjacent MPS positions (right side) @ site {}: {} - {} != 1", i, mps_it->get_position(),
    //                mps_nx->get_position()));
    //
    //            if(mps_it->get_position() != env_it->get_position())
    //                throw std::runtime_error(fmt::format("Mismatch in MPS and ENV positions (right side) @ site {}: {} != {}", i,  mps_it->get_position(),
    ...
    //
    //            if(env_it->get_position() - env_nx->get_position() != 1)
    //                throw std::runtime_error(fmt::format("Mismatch in adjacent ENV positions (right side) @ site {}: {} - {} != 1", i, env_it->get_position(),
    //                env_nx->get_position()));
    //
    //            if(env2_it->sites != env_it->sites)
    //                throw std::runtime_error(fmt::format("Mismatch in ENV2 position and ENV sites (right side) @ site {}: {} != {}", i,  env2_it->sites,
    ...
    //
    //            if(env_it->block.dimension(2) != mpo_it->get()->MPO().dimension(1))
    //                throw std::runtime_error(fmt::format("Mismatch in ENV and MPO dimensions (right side) @ site {}: {} != {}", i, env_it->block.dimension(2),
    //                mpo_it->get()->MPO().dimension(1)));
    //
    //            if(env2_it->block.dimension(2) != mpo_it->get()->MPO().dimension(1))
    //                throw std::runtime_error(fmt::format("Mismatch in ENV2 and MPO dimensions (right side) @ site {}: {} != {}", i,
    //                env2_it->block.dimension(2), mpo_it->get()->MPO().dimension(1)));
