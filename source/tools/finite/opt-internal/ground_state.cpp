...
tools::finite::opt::opt_mps tools::finite::opt::internal::ground_state_optimization(const class_tensors_finite &tensors,
                                                                                    const class_algorithm_status &status, StateRitz ritz) {
...
tools::finite::opt::opt_mps tools::finite::opt::internal::ground_state_optimization(const opt_mps &initial_mps,
                                                                                    const class_tensors_finite & tensors,
                                                                                    [[maybe_unused]] const class_algorithm_status & status,
    ...
    if(eigvecs_mps.size() >= 2)
        std::sort(eigvecs_mps.begin(), eigvecs_mps.end(), comp_energy);
    ...
    if(not eigvecs_mps.empty()) return eigvecs_mps.front();
    else  return initial_mps; // Solver failed
