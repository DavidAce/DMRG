...
tools::finite::opt::opt_mps tools::finite::opt::internal::krylov_energy_optimization(const TensorsFinite &tensors, const opt_mps &initial_mps,
    ...
    solver.eigs(matrix, nev, ncv, ritz, eig::Form::SYMM, eig::Side::R, initial_mps.get_eigval(), eig::Shinv::OFF, eig::Vecs::ON, eig::Dephase::OFF,
                residual.data());
    t_ene.toc();
    auto                 eigvecs = eig::view::get_eigvecs<cplx>(solver.result);
    ...
    for(long idx = 0; idx < eigvals.size(); idx++) {
        auto eigvec_i = tenx::TensorCast(eigvecs.col(idx), shape_mps);
        auto overlap  = std::abs(initial_mps.get_vector().dot(eigvecs.col(idx)));
        candidate_list.emplace_back(fmt::format("eigenvector {}", idx), eigvec_i, tensors.active_sites, eigvals(idx), initial_mps.get_energy_reduced(),
                                    std::nullopt, overlap, tensors.get_length());

        candidate_list.back().set_time(t_ene->get_last_interval());
        ...
        tools::log->info(
            "Candidate {:<2} overlap {:.16f} | E {:>20.16f} | E/L {:>20.16f} | eigval {:>20.16f} | Er {:>20.16f} | e_check  {:>20.16f} | variance: {:>20.16f}",
            idx, candidate_list.back().get_overlap(), candidate_list.back().get_energy(), candidate_list.back().get_energy_per_site(),
            candidate_list.back().get_eigval(), candidate_list.back().get_energy_reduced(), energy_check, std::log10(candidate_list.back().get_variance()));
    }
    ...
    Eigen::MatrixXcd H2_subspace = internal::get_multisite_hamiltonian_squared_subspace_matrix<cplx>(*tensors.model, *tensors.edges, candidate_list);
    ...
    auto optimized_tensor =
        tools::finite::opt::internal::ceres_optimize_subspace(tensors, initial_mps, candidate_list, H2_subspace, status, optType, optMode, optSpace);
