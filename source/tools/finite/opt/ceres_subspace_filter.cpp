...
void tools::finite::opt::internal::subspace::filter_candidates(std::vector<opt_mps> &candidate_list, double maximum_subspace_error, size_t max_accept) {
    ...
    for(auto &candidate : candidate_list) {
        ...
        //        tools::log->trace("Filtered {:<16}: overlap {:.16f} | energy {:>20.16f}", candidate.get_label(), candidate.get_overlap(),
...
std::optional<size_t> tools::finite::opt::internal::subspace::get_idx_to_candidate_with_lowest_variance(const std::vector<opt_mps> &candidate_list,
                                                                                                        double                      energy_llim_per_site,
                                                                                                        double                      energy_ulim_per_site) {
    if(candidate_list.empty()) return std::nullopt;
    ...
    for(const auto &[i, candidate] : iter::enumerate(candidate_list)) {
        if(not candidate.is_basis_vector) continue;
        if(candidate.get_variance() < var and candidate.get_energy_per_site() <= energy_ulim_per_site and
...
Eigen::Tensor<cplx, 3> tools::finite::opt::internal::subspace::get_tensor_in_fullspace(const std::vector<opt_mps>        &candidate_list,
                                                                                       const Eigen::VectorXcd            &subspace_vector,
                                                                                       const std::array<Eigen::Index, 3> &dims) {
    return Eigen::TensorMap<Eigen::Tensor<cplx, 3>>(subspace::get_vector_in_fullspace(candidate_list, subspace_vector).data(), dims);
}