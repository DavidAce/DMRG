...
tools::finite::opt::opt_mps tools::finite::opt::internal::krylov_variance_optimization(const TensorsFinite &tensors, const opt_mps &initial_mps,
    ...
    solver_shift.config.maxIter  = 300;
    ...
    solver_primme_sa.config.maxTime       = 60 * 60;
    ...
    solver_primme_sa.config.primme_method = "PRIMME_GD_plusK";
    ...
    if(optType == OptType::REAL) {
        ...
        if(size <= settings::precision::max_size_full_diag) {
            ...
            if(not tensors.model->is_compressed_mpo_squared()) {
                ...
                tools::log->trace(
                    "Finding excited state using shifted operator [(H-E)²-λmax], with λmax = {:.16f} | residual on ... OK: iters {} | counter {} | "
