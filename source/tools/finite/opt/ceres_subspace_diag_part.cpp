...
std::tuple<Eigen::MatrixXcd, Eigen::VectorXd>
    ...
    auto   t_part   = tid::tic_scope("part");
    ...
    auto       &t_lu = *hamiltonian.t_factorOP;
    eig::solver solver;
    ...
    for(auto nev : generate_size_list(static_cast<int>(multisite_vector.size()))) {
        ...
        reports::eigs_add_entry(nev, max_overlap, min_overlap, std::log10(subspace_error), solver.result.meta.time_total, time_ham, t_lu.get_last_interval(),
...
template std::tuple<Eigen::MatrixXcd, Eigen::VectorXd>
    tools::finite::opt::internal::subspace::find_subspace_part(const MatrixType<real> &H_local, const TensorType<cplx, 3> &multisite_mps, double energy_target,
                                                               double subspace_error_threshold, OptMode optMode, OptSpace optSpace);

template std::tuple<Eigen::MatrixXcd, Eigen::VectorXd>
    tools::finite::opt::internal::subspace::find_subspace_part(const MatrixType<cplx> &H_local, const TensorType<cplx, 3> &multisite_mps, double energy_target,
                                                               double subspace_error_threshold, OptMode optMode, OptSpace optSpace);
