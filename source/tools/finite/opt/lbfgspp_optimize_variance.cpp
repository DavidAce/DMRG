...
tools::finite::opt::opt_mps tools::finite::opt::internal::lbfgspp_optimize_variance(const TensorsFinite &tensors, const opt_mps &initial_mps,
    ...
    switch(meta.optType) {
        case OptType::CPLX: {
            ...
            double                       f;
            auto                         func_grad = [&functor, &f](const Eigen::VectorXd &x, Eigen::VectorXd &g) -> double {
            ...
            solver.minimize(func_grad, initial_guess, f);
        ...
        case OptType::REAL: {
            ...
            //            param.ftol = options.function_tolerance;
            //            param.            opt.gtol = options.gradient_tolerance;
            //            opt.gmax = options.parameter_tolerance;
            //            opt.mu = options.line_search_sufficient_function_decrease;
            //            opt.eta = options.line_search_sufficient_curvature_decrease;
            LBFGSpp::LBFGSSolver<double> solver(param);
            auto                         func_grad = [&functor](const Eigen::VectorXd &x, Eigen::VectorXd &g) -> double {
                double fx;
                functor.Evaluate(x.data(), &fx, g.data());
                return fx;
            };
            double fx = 0;
            tools::log->trace("Running BFGS real");
            solver.minimize(func_grad, initial_guess, fx);
