...
namespace tools::finite::mps {
    ...
    extern void initialize_state      (class_state_finite & state, ModelType model_type, size_t num_sites, size_t position);
    extern void normalize_state       (class_state_finite & state, std::optional<size_t> chi_lim = std::nullopt,std::optional<double> svd_threshold = std::nullopt);
    ...
    extern void move_center_point     (class_state_finite & state, std::optional<size_t> chi_lim = std::nullopt, std::optional<double> svd_threshold = std::nullopt); /*!< Move current position to the left (`direction=1`) or right (`direction=-1`), and store the **newly enlarged** environment. Turn direction around if the edge is reached. */
    extern void merge_multisite_mps   (class_state_finite & state, const Eigen::Tensor<Scalar,3> & multisite_mps, const std::list<size_t> & positions, size_t center_position, std::optional<size_t> chi_lim = std::nullopt, std::optional<double> svd_threshold = std::nullopt);
