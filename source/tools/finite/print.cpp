    ...
    for(size_t pos = 0; pos < tensors.get_length(); pos++) {
        ...
        if(pos <= tensors.get_position()) {
            tools::log->info(
                ...
                envl[0], envl[1], envl[2], mps.get_L().dimension(0), mps.spin_dim(), mps.get_chiL(), mps.get_chiR(), mpo[1], mpo[2], mpo[3], mpo[4], envr[0],
                envr[1], envr[2], tag);
        } else {
            tools::log->info(
                "Pos {:2}: ENVL [{:>3} {:>3} {:>2}] MPS [{:>2} {:>3} {:>3}] BOND [{:^4}] MPO [{:>1} {:>1} {:>1} {:>1}] ENVR [{:>3} {:>3} {:>2}] {}", pos,
                envl[0], envl[1], envl[2], mps.spin_dim(), mps.get_chiL(), mps.get_chiR(), mps.get_L().dimension(0), envr[0], mpo[0], mpo[1], mpo[2], mpo[3],
                mpo[4], envr[1], envr[2], tag);
        }
        if(tensors.state->get_mps_site(pos).isCenter())
            tools::log->info("Pos {:2}: L [{:^4}] {:>64}", pos, tensors.state->get_mps_site(pos).get_L().dimension(0), "<---- Center");
    }
}
