...
//    opt::internals::direct_optimization(state,sim_status,optType);
    ...
    switch (optType){
        case OptType::CPLX:{
            Eigen::Matrix<double,Eigen::Dynamic,1>  theta_start_cast = Eigen::Map<Eigen::VectorXd>(reinterpret_cast<double*> (theta_start.data()), 2*theta_start.size());
            auto * functor = new ceres_functor<std::complex<double>>(state, sim_status);
        ...
        case OptType::REAL:{
            ...
            auto * functor = new ceres_functor<double>(state, sim_status);
            ceres::GradientProblem problem(functor);
            tools::log->trace("Running LBFGS");
            ceres::Solve(options, problem, theta_start_cast.data(), &summary);
            niter        = (int)summary.iterations.size();
