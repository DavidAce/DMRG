...
bool class_mps_site::hasNaN() const { return Textra::hasNaN(M, "M"); }
void class_mps_site::assertValidity() const {
...
void class_mps_site::apply_mpo(const Eigen::Tensor<Scalar, 4> &mpo) {
    ...
    Eigen::Tensor<Scalar, 3> M_temp =
        M.contract(mpo, Textra::idx({0}, {2})).shuffle(Textra::array5{4, 0, 2, 1, 3}).reshape(Textra::array3{get_spin_dim(), get_chiL() * mpoDimL, get_chiR() * mpoDimR});
