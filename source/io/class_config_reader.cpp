    ...
    if(extension == ".config"){
        ...
        if (not found_file.empty()){
            try {
                ...
                tools::log->critical("Could not open file [ {} ]: {}", found_file.string(), std::string(ex.what()) );
...
bool class_config_reader::has_only_digits(const std::string &str){
    return str.find_first_not_of( "+-0123456789" ) == std::string::npos;
}

bool class_config_reader::is_parameterline(const std::string &str){
    return str.find('=') != std::string::npos;
}

std::string::size_type class_config_reader::find_comment_character(const std::string &str){
