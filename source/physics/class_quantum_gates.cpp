...
qm::Gate qm::connect(const qm::Gate &dn_gate, const qm::Gate &up_gate) {
    ...
    if(not pos_isect.empty() and not pos_nsect.empty() and inc) {
        ...
        if(dn_gate.pos.back() == pos_isect.back()) {
            ...
            Eigen::Tensor<Scalar, 2> op =
                up_gate.op.contract(dn_gate.op.reshape(dn_shp4), Textra::idx({1}, {1})).shuffle(Textra::array4{1, 0, 2, 3}).reshape(dim2);
            return qm::Gate{op, dn_gate.pos, dn_gate.dim};
            ...
            auto                     dim2 = dn_gate.shape<2>();
            Eigen::Tensor<Scalar, 2> op   = up_gate.op.contract(dn_gate.op.reshape(dn_shp4), Textra::idx({1}, {0})).reshape(dim2);
            return qm::Gate{op, dn_gate.pos, dn_gate.dim};
        } else {
            /*  Center connection
             *
            ...
            auto                     dim2 = dn_gate.shape<2>();
            Eigen::Tensor<Scalar, 2> op =
                up_gate.op.contract(dn_gate.op.reshape(dn_shp6), Textra::idx({1}, {1})).shuffle(Textra::array6{1, 0, 2, 3, 4, 5}).reshape(dim2);
            return qm::Gate{op, dn_gate.pos, dn_gate.dim};
        }
    }
    if(pos_isect.size() == 1 and pos_nsect.size() >= 2) {
        ...
        if(right) {
            ...
            Eigen::Tensor<Scalar, 2> op   = dn_gate.op.reshape(dn_shp4)
...
qm::Gate qm::trace_idx(const qm::Gate &gate, const std::vector<long> &idx) {
    tools::log->trace("Tracing gate {} | idx: {}", gate.pos, idx);
    if(idx.size() == 2) return qm::trace(gate, Textra::idx({idx[0]}, {idx[1]}));
    // idx is already pair-ordered, so when we call Textra::idx we need to undo that order
    // which explains the weird indexing in the following case
    if(idx.size() == 4) return qm::trace(gate, Textra::idx({idx[0], idx[2]}, {idx[1], idx[3]}));
...
qm::Gate qm::trace(const qm::Gate &gate, const std::array<Eigen::IndexPair<Eigen::Index>, N> &idxpairs) {
    if constexpr(N == 1) tools::log->info("Tracing:  pos {} | dim {} | pairs [{},{}]", gate.pos, gate.dim, idxpairs[0].first, idxpairs[0].second);
    if constexpr(N == 2)
        tools::log->info("Tracing:  pos {} | dim {} | pairs [{},{}][{},{}]", gate.pos, gate.dim, idxpairs[0].first, idxpairs[0].second, idxpairs[1].first,
                         idxpairs[1].second);
    ...
    else if(gate.dim.size() == 13) op_traced = Textra::trace(static_cast<T26>(gate.op.reshape(gate.shape<26>())), idxpairs).reshape(dim2);
    else if(gate.dim.size() == 14) op_traced = Textra::trace(static_cast<T28>(gate.op.reshape(gate.shape<28>())), idxpairs).reshape(dim2);
    else if(gate.dim.size() == 15) op_traced = Textra::trace(static_cast<T30>(gate.op.reshape(gate.shape<30>())), idxpairs).reshape(dim2);
    else if(gate.dim.size() == 16) op_traced = Textra::trace(static_cast<T32>(gate.op.reshape(gate.shape<32>())), idxpairs).reshape(dim2);
    /* clang-format on */
    else
